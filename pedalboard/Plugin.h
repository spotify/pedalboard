...
public:
  ...
  ChannelLayout parseAndCacheChannelLayout(
    ...
    if (!channelCountHint && lastSpec.numChannels != 0) {
      ...
      // printf("setting channel count hint to %d\n", lastSpec.numChannels);
    }

    if (!lastChannelLayout) {
      // printf("No last channel layout found\n");
    } else {
      switch (*lastChannelLayout) {
      case ChannelLayout::Interleaved:
        // printf("last channel layout given (%d, %d) input: interleaved\n",
        //        inputArray.shape(0), inputArray.shape(1));
        break;
      case ChannelLayout::NotInterleaved:
        // printf("last channel layout given (%d, %d) input: non-interleaved\n",
        //        inputArray.shape(0), inputArray.shape(1));
        break;
      }
    }
    ...
    switch (*lastChannelLayout) {
    case ChannelLayout::Interleaved:
      // printf("Parsed channel layout given (%d, %d) input: interleaved\n",
      //        inputArray.shape(0), inputArray.shape(1));
      break;
    case ChannelLayout::NotInterleaved:
      // printf("Parsed channel layout given (%d, %d) input: non-interleaved\n",
      //        inputArray.shape(0), inputArray.shape(1));
      break;
    }
