...
public:
  StreamResampler(double sourceSampleRate, double targetSampleRate,
    ...
    for (int i = 0; i < numChannels; i++) {
      resamplers[i].setQuality(quality, resamplerRatio);
      resamplers[i].reset();
    }
  ...
  long long advanceResamplerState(long long numOutputSamples) {
    std::tuple<double, long long> subSamplePosAndNumSamplesUsed =
        resamplers[0].calculateSubSamplePosition(1.0, resamplerRatio,
                                                 numOutputSamples);

    float zero = 0.0f;
    for (auto &resampler : resamplers) {
      // This effectively sets the new subsample position:
      resampler.process(std::get<0>(subSamplePosAndNumSamplesUsed), &zero,
                        &zero, 1);
    }

    totalSamplesOutput += numOutputSamples;
    totalSamplesInput += std::get<1>(subSamplePosAndNumSamplesUsed);

    return std::get<1>(subSamplePosAndNumSamplesUsed);
  }
