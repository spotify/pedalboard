...
public:
    ...
    PythonInputStream(py::object fileLike): fileLike(fileLike) {
        if (!py::hasattr(fileLike, "read") || !py::hasattr(fileLike, "seek") || !py::hasattr(fileLike, "tell") || !py::hasattr(fileLike, "seekable")) {
            throw py::type_error("Expected a file-like object (with read, seek, seekable, and tell methods).");
        }
    }
    ...
    bool isSeekable() {
        try {
            ...
            printf("[PythonInputStream] Error raised\n");
    ...
    juce::int64 getTotalLength() {
        ...
        // TODO: Try reading a couple of Python properties that may contain the total length:
        // urllib3.response.HTTPResponse provides `length_remaining`

        try {
            if (!fileLike.attr("seekable")().cast<bool>()) {
                printf("[PythonInputStream] getTotalLength => -1\n");
                return -1;
            }
        } catch (py::error_already_set) {
            printf("[PythonInputStream] Error raised\n");
        ...
        printf("[PythonInputStream] getTotalLength => %d\n", totalLength);
        return totalLength;
    }
    ...
    int read (void* buffer, int bytesToRead)
    {
        ...
        std::memcpy((char *)buffer, (char *)resultAsString.data(), resultAsString.size());

        int bytesCopied = resultAsString.size();
        printf("[PythonInputStream] read(%p, %d) => %d\n", buffer, bytesToRead, bytesCopied);
    ...
    juce::int64 getPosition() {
        printf("[PythonInputStream] getPosition() => %d\n", fileLike.attr("tell")().cast<juce::int64>());
        try {
            py::gil_scoped_acquire acquire;
    ...
    {   
        printf("[PythonInputStream] setPosition(%d)\n", pos);
        try {
            py::gil_scoped_acquire acquire;
    ...
    void raisePythonErrorIfNecessary() {
        py::gil_scoped_acquire acquire;
        if (pythonErrorRaised) throw py::error_already_set();
    }
