...
cd pedalboard
pip3 install nanobind scikit-build-core[pyproject] tox
pip3 install .
```
...
> 
> # If using GCC:
> rm -rf build && CC="ccache gcc" CXX="ccache g++" DEBUG=1 pip3 install --no-build-isolation -ve .
> 
> # ...or if using Clang:
> rm -rf build && CC="ccache clang" CXX="ccache clang++" DEBUG=1 pip3 install --no-build-isolation -ve .
> ```

By default, all `.cpp` and `.mm` files in the `pedalboard` directory (or subdirectories) will be automatically compiled by CMake.

Pedalboard ships with `.pyi` files to allow for type hints in text editors and via MyPy. These stub files are automatically generated by Nanobind during the build process. After building, you can verify the stubs are valid:
