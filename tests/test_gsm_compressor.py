...
@pytest.mark.parametrize("sample_rate", [8000, 32001.2345, 44100, 48000])
@pytest.mark.parametrize("buffer_size", [32, 160, 1_000_000])
...
@pytest.mark.parametrize("sample_rate", [44100])
@pytest.mark.parametrize(
    "quality",
    [Resample.Quality.ZeroOrderHold, Resample.Quality.WindowedSinc],
)
@pytest.mark.parametrize("num_channels", [1, 2])
def test_gsm_compressor_invariant_to_buffer_size(
    ...
    compressor = GSMFullRateCompressor(quality=quality)
    compressed = [
        compressor(signal, sample_rate, buffer_size=buffer_size) for buffer_size in (32, 7000, 8192)
    ]
