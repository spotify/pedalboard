...
static void serial_threads(void *(*work)(char *), char *jobdata, size_t elsize,
                           int njobs, void *data) {
  int i;
  (void)data; /* unused */
  for (i = 0; i < njobs; ++i)
...
void useropt(const char *arg) {
  ...
  if (!strcmp(arg, "patient"))
    the_flags |= FFTW_PATIENT;
  else if (!strcmp(arg, "estimate"))
    the_flags |= FFTW_ESTIMATE;
  else if (!strcmp(arg, "estimatepat"))
    the_flags |= FFTW_ESTIMATE_PATIENT;
  else if (!strcmp(arg, "exhaustive"))
    the_flags |= FFTW_EXHAUSTIVE;
  else if (!strcmp(arg, "unaligned"))
    the_flags |= FFTW_UNALIGNED;
  else if (!strcmp(arg, "nosimd"))
    the_flags |= FFTW_NO_SIMD;
  else if (!strcmp(arg, "noindirectop"))
    the_flags |= FFTW_NO_INDIRECT_OP;
  else if (!strcmp(arg, "wisdom-only"))
    the_flags |= FFTW_WISDOM_ONLY;
  else if (sscanf(arg, "flag=%d", &x) == 1)
    the_flags |= x;
  else if (sscanf(arg, "bflag=%d", &x) == 1)
    the_flags |= 1U << x;
  else if (!strcmp(arg, "paranoid"))
    paranoid = 1;
  else if (!strcmp(arg, "wisdom"))
    usewisdom = 1;
  else if (!strcmp(arg, "amnesia"))
    amnesia = 1;
...
#endif
  else if (sscanf(arg, "nthreads=%d", &x) == 1)
    nthreads = x;
#ifdef FFTW_RANDOM_ESTIMATOR
  else if (sscanf(arg, "eseed=%d", &x) == 1)
    FFTW(random_estimate_seed) = x;
#endif
...
void rdwisdom(void) {
  ...
  if (threads_ok) {
    ...
    fprintf(stderr,
            "bench: WARNING - nthreads = %d, but threads not supported\n",
            nthreads);
    nthreads = 1;
  }
#endif
  ...
  if (success) {
    if (verbose > 1)
      printf("READ WISDOM (%g seconds): ", tim);
...
void wrwisdom(void) {
  ...
  tim = timer_stop(USER_TIMER);
  if (verbose > 1)
    printf("write wisdom took %g seconds\n", tim);
}
...
static unsigned preserve_input_flags(bench_problem *p) {
   ...
   */
  if (p->kind == PROBLEM_REAL && p->sign > 0 && !p->in_place && p->sz->rnk > 1)
    p->destroy_input = 1;
...
int can_do(bench_problem *p) {
  ...
  tim = timer_stop(USER_TIMER);
  if (verbose > 2)
    printf("estimate-planner time: %g s\n", tim);
