...
static void print(const problem *ego_, printer *p) {
  ...
  int i;
  p->print(p, "(mpi-dft %d %d %d ", ego->I == ego->O, X(ialignment_of)(ego->I),
...
static const problem_adt padt = {PROBLEM_MPI_DFT, hash, zero, print, destroy};

problem *XM(mkproblem_dft)(const dtensor *sz, INT vn, R *I, R *O, MPI_Comm comm,
...
problem *XM(mkproblem_dft_d)(dtensor *sz, INT vn, R *I, R *O, MPI_Comm comm,
                             int sign, unsigned flags) {
