...
static plan *mkplan(const solver *ego, const problem *p_, planner *plnr) {
  ...
  if (my_pe == 0 && p->vn > 0) {
    ...
    rdft_kind *kind = (rdft_kind *)MALLOC(sizeof(rdft_kind) * rnk, PROBLEMS);
    ...
    cld = X(mkplan_d)(plnr, X(mkproblem_rdft_d)(sz, X(mktensor_1d)(p->vn, 1, 1),
                                                p->I, p->O, kind));
    X(ifree0)(kind);
  } else { /* idle process: make nop plan */
    cld = X(mkplan_d)(
        plnr, X(mkproblem_rdft_0_d)(X(mktensor_1d)(0, 0, 0), p->I, p->O));
  }
...
void XM(rdft_serial_register)(planner *p) { REGISTER_SOLVER(p, mksolver()); }
