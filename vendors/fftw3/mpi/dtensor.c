...
dtensor *XM(mkdtensor)(int rnk) {
  ...
  if (FINITE_RNK(rnk) && rnk > 1)
    x = (dtensor *)MALLOC(sizeof(dtensor) + (rnk - 1) * sizeof(ddim), TENSORS);
  else
    x = (dtensor *)MALLOC(sizeof(dtensor), TENSORS);
#elif defined(STRUCT_HACK_C99)
  if (FINITE_RNK(rnk))
    x = (dtensor *)MALLOC(sizeof(dtensor) + rnk * sizeof(ddim), TENSORS);
  else
    x = (dtensor *)MALLOC(sizeof(dtensor), TENSORS);
#else
  x = (dtensor *)MALLOC(sizeof(dtensor), TENSORS);
...
int XM(dtensor_validp)(const dtensor *sz) {
  ...
  if (FINITE_RNK(sz->rnk))
    for (i = 0; i < sz->rnk; ++i)
      if (sz->dims[i].n < 0 || sz->dims[i].b[IB] <= 0 || sz->dims[i].b[OB] <= 0)
        return 0;
  return 1;
}
...
void XM(dtensor_print)(const dtensor *t, printer *p) {
  if (FINITE_RNK(t->rnk)) {
    ...
    for (i = 0; i < t->rnk; ++i) {
      ...
      p->print(p, "%s(%D %D %D)", first ? "" : " ", d->n, d->b[IB], d->b[OB]);
      first = 0;
    }
    p->print(p, ")");
  } else {
