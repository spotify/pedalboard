...
static int applicable(const S *ego, const problem *p_, const planner *plnr) {
  ...
  return (
      1 && p->sz->rnk == 1 && !(p->flags & ~RANK1_BIGVEC_ONLY) &&
...
static plan *mkplan(const solver *ego_, const problem *p_, planner *plnr) {
  ...
  cldt_before = X(mkplan_d)(
      plnr, XM(mkproblem_transpose)(nx, ny, vn * 2, I = p->I, O = p->O,
  ...
  cld = X(mkplan_d)(
      ...
      X(mkproblem_dft_d)(X(mktensor_1d)(nx, vn * 2, vn * 2),
                         X(mktensor_2d)(yb, vn * 2 * nx, vn * 2 * nx, vn, 2, 2),
                         ro, io, ri, ii));
  if (XM(any_true)(!cld, p->comm))
    goto nada;

  cldt_after = X(mkplan_d)(
      plnr, XM(mkproblem_transpose)(ny, nx, vn * 2, I, O, yblock,
