...
static plan *mkplan(const solver *ego, const problem *p_, planner *plnr) {
  ...
  if (my_pe == 0 && p->vn > 0) {
    ...
    cld = X(mkplan_d)(plnr,
                      X(mkproblem_rdft2_d)(sz, X(mktensor_1d)(p->vn, ivs, ovs),
                                           r0, r1, cr, ci, p->kind));
  } else { /* idle process: make nop plan */
    cld = X(mkplan_d)(plnr, X(mkproblem_rdft2_d)(X(mktensor_0d)(),
                                                 X(mktensor_1d)(0, 0, 0), cr,
                                                 ci, cr, ci, HC2R));
  }
...
void XM(rdft2_serial_register)(planner *p) { REGISTER_SOLVER(p, mksolver()); }
