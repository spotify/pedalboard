...
static int applicable(const S *ego, const problem *p_, const planner *plnr,
  ...
  return (
      ...
      && (*r = XM(choose_radix)(p->sz->dims[0], n_pes, p->flags, p->sign,
                                rblock, mblock))
      ...
      && (!NO_UGLYP(plnr) || !(*r == n_pes && ego->apply == apply_ddft_first) ||
          !(p->sz->dims[0].n / *r == n_pes && ego->apply == apply_ddft_last)));
}
...
static void print(const plan *ego_, printer *p) {
  const P *ego = (const P *)ego_;
  p->print(p, "(mpi-dft-rank1/%D%s%s%(%p%)%(%p%)%(%p%))", ego->r,
           ego->super.apply == apply_ddft_first ? "/first" : "/last",
           ego->preserve_input == 2 ? "/p" : "", ego->cld_ddft, ego->cld_dft,
           ego->cldt);
}
...
static plan *mkplan(const solver *ego_, const problem *p_, planner *plnr) {
  ...
  dtensor *sz;
  static const plan_adt padt = {XM(dft_solve), awake, print, destroy};
  ...
  if ((ddft_first = (ego->apply == apply_ddft_first))) {
    rp = r;
    mp = m;
    mpblock[IB] = mblock[IB];
    mpblock[OB] = mblock[OB];
    mpb = XM(block)(mp, mpblock[OB], my_pe);
  } else {
    rp = m;
    mp = r;
    mpblock[IB] = rblock[IB];
    mpblock[OB] = rblock[OB];
  ...
  O = p->O;
  cld_ddft =
      X(mkplan_d)(plnr, XM(mkproblem_dft_d)(sz, rp * p->vn, I, O, p->comm,
                                            p->sign, RANK1_BIGVEC_ONLY));
  if (XM(any_true)(!cld_ddft, p->comm))
    goto nada;
  ...
  cld_dft = X(mkplan_d)(
      plnr, X(mkproblem_dft_d)(X(mktensor_1d)(rp, p->vn * 2, p->vn * 2),
                               X(mktensor_1d)(p->vn, 2, 2), ri, ii, ro, io));
  if (XM(any_true)(!cld_dft, p->comm))
    goto nada;
  ...
  if (!p->flags) { /* !(SCRAMBLED_IN or SCRAMBLED_OUT) */
    ...
    cldt = X(mkplan_d)(
        plnr, XM(mkproblem_transpose)(
                  m, r, p->vn * 2, I, O, ddft_first ? mblock[OB] : mblock[IB],
                  ddft_first ? rblock[OB] : rblock[IB], p->comm, 0));
    if (XM(any_true)(!cldt, p->comm))
      goto nada;
  }
