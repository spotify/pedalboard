...
static void transpose_chunks(int *sched, int n_pes, int my_pe, INT *sbs,
                             INT *sbo, INT *rbs, INT *rbo, MPI_Comm comm, R *I,
                             R *O) {
  if (sched) {
    int i;
    MPI_Status status;
    ...
    if (I == O) {
      ...
      for (i = 0; i < n_pes; ++i) {
        ...
        if (my_pe == pe) {
          ...
          MPI_Sendrecv(buf, (int)(sbs[pe]), FFTW_MPI_TYPE, pe,
                       (my_pe * n_pes + pe) & 0x7fff, O + rbo[pe],
                       (int)(rbs[pe]), FFTW_MPI_TYPE, pe,
                       (pe * n_pes + my_pe) & 0x7fff, comm, &status);
        }
      }
      ...
      for (i = 0; i < n_pes; ++i) {
        ...
        if (my_pe == pe)
          ...
          MPI_Sendrecv(I + sbo[pe], (int)(sbs[pe]), FFTW_MPI_TYPE, pe,
                       (my_pe * n_pes + pe) & 0x7fff, O + rbo[pe],
                       (int)(rbs[pe]), FFTW_MPI_TYPE, pe,
                       (pe * n_pes + my_pe) & 0x7fff, comm, &status);
      }
...
static void apply(const plan *ego_, R *I, R *O) {
  ...
  if (cld1) {
    ...
    transpose_chunks(ego->sched, ego->n_pes, ego->my_pe, ego->send_block_sizes,
                     ego->send_block_offsets, ego->recv_block_sizes,
                     ego->recv_block_offsets, ego->comm, O, I);
  } else if (ego->preserve_input) {
    /* transpose chunks globally */
    transpose_chunks(ego->sched, ego->n_pes, ego->my_pe, ego->send_block_sizes,
                     ego->send_block_offsets, ego->recv_block_sizes,
                     ego->recv_block_offsets, ego->comm, I, O);
    ...
    transpose_chunks(ego->sched, ego->n_pes, ego->my_pe, ego->send_block_sizes,
                     ego->send_block_offsets, ego->recv_block_sizes,
                     ego->recv_block_offsets, ego->comm, I, I);
  }
  ...
  if (cld2rest) {
    cld2rest->apply(ego->cld2rest, I + ego->rest_Ioff, O + ego->rest_Ooff);
...
int XM(mkplans_posttranspose)(const problem_mpi_transpose *p, planner *plnr,
                              R *I, R *O, int my_pe, plan **cld2,
                              plan **cld2rest, plan **cld3, INT *rest_Ioff,
                              INT *rest_Ooff) {
  INT vn = p->vn;
  INT b = p->block;
  ...
  if (!(p->flags & TRANSPOSED_OUT) && (nxr == 0 || I != O)) {
    ...
    *cld2 = X(mkplan_f_d)(
        ...
        X(mkproblem_rdft_0_d)(
            X(mktensor_3d)(nxb, bt * b, b, bt, b, nx, b, 1, 1), I, O),
        0, 0, NO_SLOW);
    if (!*cld2)
      goto nada;
    ...
    if (nxr > 0) {
      ...
      *rest_Ooff = nxb * b;
      b = nxr * vn;
      *cld2rest = X(mkplan_f_d)(
          plnr,
          X(mkproblem_rdft_0_d)(X(mktensor_2d)(bt, b, nx, b, 1, 1),
    ...
    *cld2 =
        X(mkplan_f_d)(plnr,
                      X(mkproblem_rdft_0_d)(
                          X(mktensor_4d)(nxb, bt * b * vn, bt * b * vn, bt,
                                         b * vn, vn, b, vn, bt * vn, vn, 1, 1),
                          I, O),
                      0, 0, NO_SLOW);
    if (!*cld2)
      goto nada;
    ...
    *cld2rest = X(mkplan_f_d)(
        plnr,
        X(mkproblem_rdft_0_d)(
            X(mktensor_3d)(bt, nxr * vn, vn, nxr, vn, bt * vn, vn, 1, 1),
            I + *rest_Ioff, O + *rest_Ooff),
        0, 0, NO_SLOW);
    if (!*cld2rest)
      goto nada;
    ...
    if (!(p->flags & TRANSPOSED_OUT)) {
      *cld3 = X(mkplan_f_d)(
          plnr,
          X(mkproblem_rdft_0_d)(
              X(mktensor_3d)(p->nx, bt * vn, vn, bt, vn, p->nx * vn, vn, 1, 1),
              O, O),
          0, 0, NO_SLOW);
      if (!*cld3)
        goto nada;
...
static plan *mkplan(const solver *ego_, const problem *p_, planner *plnr) {
  ...
  R *I, *O;
  static const plan_adt padt = {XM(transpose_solve), awake, print, destroy};
  ...
  if (!(p->flags & TRANSPOSED_IN)) { /* b x ny x vn -> ny x b x vn */
    cld1 = X(mkplan_f_d)(
        plnr,
        X(mkproblem_rdft_0_d)(
            X(mktensor_3d)(b, p->ny * vn, vn, p->ny, vn, b * vn, vn, 1, 1), I,
            O),
        0, 0, NO_SLOW);
  ...
  if (XM(any_true)(!XM(mkplans_posttranspose)(p, plnr, I, O, my_pe, &cld2,
                                              &cld2rest, &cld3, &rest_Ioff,
                                              &rest_Ooff),
                   p->comm))
    goto nada;
  ...
  if (cld1)
    X(ops_add2)(&cld1->ops, &pln->super.super.ops);
  if (cld2)
    X(ops_add2)(&cld2->ops, &pln->super.super.ops);
  if (cld2rest)
    X(ops_add2)(&cld2rest->ops, &pln->super.super.ops);
  if (cld3)
    X(ops_add2)(&cld3->ops, &pln->super.super.ops);
