...
static int applicable(const S *ego, const problem *p_, const planner *plnr) {
  ...
  return (1 && p->sz->rnk == 1 && !(p->flags & ~RANK1_BIGVEC_ONLY) &&
		 ...
		 || (p->flags & RANK1_BIGVEC_ONLY))
          ...
          &&
          XM(rearrange_applicable)(ego->rearrange, p->sz->dims[0], p->vn, n_pes)
...
static plan *mkplan(const solver *ego_, const problem *p_, planner *plnr) {
  ...
  cldt_before = X(mkplan_d)(
      plnr, XM(mkproblem_transpose)(nx, ny, vn, I = p->I, O = p->O,
                                    p->sz->dims[0].b[IB], yblock, p->comm, 0));
  if (XM(any_true)(!cldt_before, p->comm))
    goto nada;
  ...
  cld = X(mkplan_d)(plnr, X(mkproblem_rdft_1_d)(
                              X(mktensor_1d)(nx, vn, vn),
                              X(mktensor_2d)(yb, vn * nx, vn * nx, vn, 1, 1), O,
                              I, p->kind[0]));
  if (XM(any_true)(!cld, p->comm))
    goto nada;

  cldt_after = X(mkplan_d)(
      plnr, XM(mkproblem_transpose)(ny, nx, vn, I, O, yblock,
                                    p->sz->dims[0].b[OB], p->comm, 0));
  if (XM(any_true)(!cldt_after, p->comm))
    goto nada;
