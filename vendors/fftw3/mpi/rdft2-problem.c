...
static void print(const problem *ego_, printer *p) {
  ...
  int i;
  p->print(p, "(mpi-rdft2 %d %d %d ", ego->I == ego->O,
           X(ialignment_of)(ego->I), X(ialignment_of)(ego->O));
...
static const problem_adt padt = {PROBLEM_MPI_RDFT2, hash, zero, print, destroy};

problem *XM(mkproblem_rdft2)(const dtensor *sz, INT vn, R *I, R *O,
...
problem *XM(mkproblem_rdft2_d)(dtensor *sz, INT vn, R *I, R *O, MPI_Comm comm,
