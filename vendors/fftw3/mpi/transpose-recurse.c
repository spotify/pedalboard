...
static void apply(const plan *ego_, R *I, R *O) {
  ...
  if (cldtr)
    cldtr->apply((plan *)cldtr, O, I);

  cldtm = (plan_rdft *)ego->cldtm;
  if (cldtm)
    cldtm->apply((plan *)cldtm, I, O);
}
...
static INT transpose_space(INT nx, INT ny, INT block, INT tblock, int pe) {
  return X(imax)(XM(block)(nx, block, pe) * ny, nx * XM(block)(ny, tblock, pe));
}
...
static int enough_space(INT nx, INT ny, INT block, INT tblock, int r,
                        int n_pes) {
  int pe;
  int m = n_pes / r;
...
static void print(const plan *ego_, printer *p) {
  const P *ego = (const P *)ego_;
  p->print(p, "(mpi-transpose-recurse/%s/%d%s%(%p%)%(%p%)%(%p%))", ego->nam,
           ego->r, ego->preserve_input == 2 ? "/p" : "", ego->cld1, ego->cldtr,
           ego->cldtm);
}
...
static plan *mkplan(const solver *ego_, const problem *p_, planner *plnr) {
  ...
  if (p->flags & TRANSPOSED_IN) {
    ...
    cld1 = X(mkplan_f_d)(
        ...
        X(mkproblem_rdft_0_d)(
            X(mktensor_3d)(m, r * vn, vn, r, vn, m * vn, vn, 1, 1), I, O),
        0, 0, NO_SLOW);
    ...
    cld1 = X(mkplan_f_d)(
        plnr,
        X(mkproblem_rdft_0_d)(X(mktensor_5d)(b, m * r * bt * vn, vn, m,
                                             r * bt * vn, bt * b * vn, r,
                                             bt * vn, m * bt * b * vn, bt, vn,
                                             b * vn, vn, 1, 1),
                              I, O),
        0, 0, NO_SLOW);
    ...
    cld1 = X(mkplan_f_d)(
        plnr,
        X(mkproblem_rdft_0_d)(X(mktensor_4d)(m, r * vn, vn, r, vn, m * vn, vn,
                                             1, 1, b, np * vn, np * vn),
                              I, O),
        0, 0, NO_SLOW);
  }
  ...
  if (b)
    cldtr = X(mkplan_d)(
        plnr, XM(mkproblem_transpose)(
                  b, p->ny, p->vn, O, I, p->block, m * p->tblock, comm2,
                  p->I != p->O ? TRANSPOSED_IN : (p->flags & TRANSPOSED_IN)));
  MPI_Comm_free(&comm2);
  if (XM(any_true)(b && !cldtr, p->comm))
    goto nada;
  ...
  if (b)
    cldtm = X(mkplan_d)(plnr, XM(mkproblem_transpose)(
                                  p->nx, b, p->vn, I, O, r * p->block,
                                  p->tblock, comm2,
                                  TRANSPOSED_IN | (p->flags & TRANSPOSED_OUT)));
  MPI_Comm_free(&comm2);
  if (XM(any_true)(b && !cldtm, p->comm))
    goto nada;
  ...
  if (cldtr)
    X(ops_add2)(&cldtr->ops, &pln->super.super.ops);
  if (cldtm)
    X(ops_add2)(&cldtm->ops, &pln->super.super.ops);
...
static solver *mksolver(int preserve_input, int (*radix)(int np),
                        const char *nam) {
