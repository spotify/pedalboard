...
int XM(dft_serial_applicable)(const problem_mpi_dft *p) {
  return (1 && p->flags == 0 /* TRANSPOSED/SCRAMBLED_IN/OUT not supported */
          &&
          ((XM(is_local)(p->sz, IB) && XM(is_local)(p->sz, OB)) || p->vn == 0));
}
...
static plan *mkplan(const solver *ego, const problem *p_, planner *plnr) {
  ...
  if (my_pe == 0 && p->vn > 0) {
    ...
    for (i = rnk - 1; i > 0; --i) {
      sz->dims[i - 1].is = sz->dims[i - 1].os = sz->dims[i].is * sz->dims[i].n;
      sz->dims[i - 1].n = p->sz->dims[i - 1].n;
    }

    cld = X(mkplan_d)(plnr, X(mkproblem_dft_d)(sz, X(mktensor_1d)(p->vn, 2, 2),
                                               ri, ii, ro, io));
  } else { /* idle process: make nop plan */
    cld = X(mkplan_d)(plnr, X(mkproblem_dft_d)(X(mktensor_0d)(),
                                               X(mktensor_1d)(0, 0, 0), ri, ii,
                                               ro, io));
  }
...
void XM(dft_serial_register)(planner *p) { REGISTER_SOLVER(p, mksolver()); }
