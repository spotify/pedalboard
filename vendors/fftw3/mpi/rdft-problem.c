...
static void print(const problem *ego_, printer *p) {
  ...
  int i;
  p->print(p, "(mpi-rdft %d %d %d ", ego->I == ego->O, X(ialignment_of)(ego->I),
...
static const problem_adt padt = {PROBLEM_MPI_RDFT, hash, zero, print, destroy};
...
problem *XM(mkproblem_rdft)(const dtensor *sz, INT vn, R *I, R *O,
  ...
  ego = (problem_mpi_rdft *)X(mkproblem)(
      sizeof(problem_mpi_rdft) + sizeof(rdft_kind) * (rnk > 0 ? rnk - 1 : 0),
      &padt);
#elif defined(STRUCT_HACK_C99)
  ego = (problem_mpi_rdft *)X(mkproblem)(
      sizeof(problem_mpi_rdft) + sizeof(rdft_kind) * rnk, &padt);
#else
