...
static int applicable(const S *ego, const problem *p_, const planner *plnr) {
  ...
  return (
      1 && p->sz->rnk > 1 && p->flags == TRANSPOSED_OUT &&
...
static plan *mkplan(const solver *ego_, const problem *p_, planner *plnr) {
    ...
    nrest *= sz->dims[i].n;
  {
    ...
    cld1 = X(mkplan_d)(
        plnr, X(mkproblem_dft_d)(sz, X(mktensor_2d)(b, is, is, p->vn, 2, 2), ri,
                                 ii, ro, io));
  ...
  nrest *= p->vn;
  cldt = X(mkplan_d)(
      plnr, XM(mkproblem_transpose)(p->sz->dims[0].n, p->sz->dims[1].n,
                                    nrest * 2, I, O, p->sz->dims[0].b[IB],
                                    p->sz->dims[1].b[OB], p->comm, 0));
  ...
  X(extract_reim)(p->sign, O, &ro, &io);
  {
    ...
    cld2 = X(mkplan_d)(
        plnr, X(mkproblem_dft_d)(
                  ...
                  X(mktensor_2d)(b, is, is, nrest, 2, 2), ro, io, ro, io));
