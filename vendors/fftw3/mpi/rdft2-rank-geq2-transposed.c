...
static int applicable(const S *ego, const problem *p_, const planner *plnr) {
  ...
  return (
      ...
      ((p->flags == TRANSPOSED_OUT && p->kind == R2HC &&
        XM(is_local_after)(1, p->sz, IB) && XM(is_local_after)(2, p->sz, OB) &&
        XM(num_blocks)(p->sz->dims[0].n, p->sz->dims[0].b[OB]) == 1) ||
       (p->flags == TRANSPOSED_IN && p->kind == HC2R &&
        XM(is_local_after)(1, p->sz, OB) && XM(is_local_after)(2, p->sz, IB) &&
...
static void print(const plan *ego_, printer *p) {
  ...
  p->print(p, "(mpi-rdft2-rank-geq2-transposed%s%(%p%)%(%p%)%(%p%))",
           ego->preserve_input == 2 ? "/p" : "", ego->cld1, ego->cldt,
           ego->cld2);
}
...
static plan *mkplan(const solver *ego_, const problem *p_, planner *plnr) {
  ...
  int i, my_pe, n_pes;
  INT nrest, n1, b1;
  static const plan_adt padt = {XM(rdft2_solve), awake, print, destroy};
  block_kind k1, k2;
    ...
    nrest *= sz->dims[i].n;
  {
    ...
    cld1 = X(mkplan_d)(plnr, X(mkproblem_rdft2_d)(
                                 sz, X(mktensor_2d)(b, is, is, p->vn, ivs, ovs),
                                 r0, r1, cr, ci, p->kind));
  ...
  if (p->kind == R2HC)
    cldt = X(mkplan_d)(
        plnr, XM(mkproblem_transpose)(p->sz->dims[0].n, n1, nrest * 2, I, O,
                                      p->sz->dims[0].b[IB], b1, p->comm, 0));
  else
    cldt = X(mkplan_d)(
        plnr, XM(mkproblem_transpose)(n1, p->sz->dims[0].n, nrest * 2, I, O, b1,
                                      p->sz->dims[0].b[OB], p->comm, 0));
...

  {
    ...
    cld2 = X(mkplan_d)(
        plnr, X(mkproblem_dft_d)(
                  X(mktensor_1d)(p->sz->dims[0].n, nrest * 2, nrest * 2),
                  X(mktensor_2d)(b, is, is, nrest, 2, 2), ri, ii, ro, io));
