...
static trigreal cos00(int i, int j, int n) { return cos2pi(i * j, n); }
...
static trigreal sin00(int i, int j, int n) { return sin2pi(i * j, n); }
...
static void rarolr(R *b, R *a, int n, int nb, int na, r2r_kind_t k) {
  ...
  for (ib = 0; ib < nb; ++ib) {
      ...
      if (n > 2) {
        if (n % 2 == 0)
          ...
          for (ia = 0; ia < na; ++ia) {
            b[(ib * n + n - 1) * na + ia] = 0.0;
            b[(ib * n + 0) * na + ia] += a[(ib * n + 1) * na + ia];
            b[(ib * n + n / 2) * na + ia] += +a[(ib * n + n / 2) * na + ia] -
                                             a[(ib * n + n / 2 + 1) * na + ia];
            b[(ib * n + n / 2 + 1) * na + ia] +=
                -a[(ib * n + n / 2 + 1) * na + ia] -
                a[(ib * n + n / 2) * na + ia];
    ...
    mirrors:
      ...
      for (ia = 0; ia < na; ++ia)
        b[(ib * n) * na + ia] += isL0 * a[(ib * n) * na + ia] +
...
static void raphase_shift(R *b, R *a, int n, int nb, int na, int n0, int k0,
                          trigfun t) {
  int j, jb, ja;
...
static void impulse_response(int rnk, dim_stuff *d, R impulse_amp, R *A,
                             int N) {
  if (rnk == 0)
    A[0] = impulse_amp;
  else {
    ...
    for (i = 0; i < d->n; ++i) {
      impulse_response(rnk - 1, d + 1,
                       impulse_amp * d->ti(d->i0, d->k0 + i, d->n0), A + i * N,
                       N);
    }
  }
}
...
static double rimpulse(dim_stuff *d, R impulse_amp, int n, int vecn, info *nfo,
                       R *inA, R *inB, R *inC, R *outA, R *outB, R *outC,
...
static double t_shift(int n, int vecn, info *nfo, R *inA, R *inB, R *outA,
                      R *outB, R *tmp, int rounds, double tol, dim_stuff *d) {
  double e = 0.0;
  ...
  for (dim = 0; dim < sz->rnk; ++dim) {
    ...
    for (j = 0; j < rounds; ++j) {
      ...
      for (i = 0; i < vecn; ++i) {
        rarolr(inB + i * n, inA + i * n, ncur, nb, na, nfo->p->k[dim]);
      }
      dofft(nfo, inA, outA);
      dofft(nfo, inB, outB);
      for (i = 0; i < vecn; ++i)
        raphase_shift(tmp + i * n, outA + i * n, ncur, nb, na, d[dim].n0,
                      d[dim].k0, d[dim].ts);
...
void verify_r2r(bench_problem *p, int rounds, double tol, errors *e) {
  ...
  for (i = 0; i < p->sz->rnk; ++i) {
    ...
    if (p->k[i] > R2R_DHT)
      n0 = 2 *
           (n0 +
            (p->k[i] == R2R_REDFT00 ? -1 : (p->k[i] == R2R_RODFT00 ? 1 : 0)));
  ...
  e->i = rimpulse(d, impulse_amp, n, vecn, &nfo, inA, inB, inC, outA, outB,
                  outC, tmp, rounds, tol);
  e->l = rlinear(N, &nfo, inA, inB, inC, outA, outB, outC, tmp, rounds, tol);
  e->s = t_shift(n, vecn, &nfo, inA, inB, outA, outB, tmp, rounds, tol, d);
...
static void r2r_apply(dofft_closure *k_, bench_complex *in,
                      bench_complex *out) {
