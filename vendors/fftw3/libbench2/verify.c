...
void verify_problem(bench_problem *p, int rounds, double tol) {
  ...
  case PROBLEM_COMPLEX:
    verify_dft(p, rounds, tol, &e);
    break;
  case PROBLEM_REAL:
    verify_rdft2(p, rounds, tol, &e);
    break;
  case PROBLEM_R2R:
    verify_r2r(p, rounds, tol, &e);
    break;
  }
...
static void do_accuracy(bench_problem *p, int rounds, int impulse_rounds) {
  ...
  switch (p->kind) {
  case PROBLEM_COMPLEX:
    accuracy_dft(p, rounds, impulse_rounds, t);
    break;
  case PROBLEM_REAL:
    accuracy_rdft2(p, rounds, impulse_rounds, t);
    break;
  case PROBLEM_R2R:
    accuracy_r2r(p, rounds, impulse_rounds, t);
    break;
  }
