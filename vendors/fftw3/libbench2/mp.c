...
static void cpy(const N a, N b) { *b = *a; }
...
/* 2 pi */
static const N n2pi = {
    {1, 1, {18450, 59017, 1760, 5212, 9779, 4518, 2886, 54545, 18558, 6}}};
...
static void bluestein(int n, N *a) {
  ...
  for (i = 0; i < n; ++i)
    cmulj(w[2 * i], w[2 * i + 1], a[2 * i], a[2 * i + 1], b[2 * i],
          b[2 * i + 1]);
  ...
  for (i = 0; i < nb; ++i)
    cmul(b[2 * i], b[2 * i + 1], y[2 * i], y[2 * i + 1], b[2 * i],
         b[2 * i + 1]);
  fft0(nb, b, 1);

  for (i = 0; i < n; ++i) {
    cmulj(w[2 * i], w[2 * i + 1], b[2 * i], b[2 * i + 1], a[2 * i],
          a[2 * i + 1]);
...
void fftaccuracy(int n, bench_complex *a, bench_complex *ffta, int sign,
                 double err[6]) {
...
void fftaccuracy_done(void) {
  if (cached_bluestein_w)
    bench_free(cached_bluestein_w);
  if (cached_bluestein_y)
    bench_free(cached_bluestein_y);
