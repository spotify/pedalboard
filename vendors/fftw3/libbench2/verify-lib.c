...
static double aerror(C *a, C *b, int n) {
  if (n > 0) {
    ...
    for (i = 0; i < n; ++i) {
      e = dmax(e, norm2(c_re(a[i]) - c_re(b[i]), c_im(a[i]) - c_im(b[i])));
      mag = dmax(mag, dmin(norm2(c_re(a[i]), c_im(a[i])),
...
#endif
double mydrand(void) { return drand48() - 0.5; }
#else
double mydrand(void) {
  double d = rand();
...
static void assign_conj(C *Ac, C *A, int rank, const bench_iodim *dim,
                        int stride) {
  if (rank == 0) {
    c_re(*Ac) = c_re(*A);
    c_im(*Ac) = -c_im(*A);
...

    {
      ...
      for (i = 0; i < N; ++i)
        ovtpvt_err("%8d %16.12f %16.12f   %16.12f %16.12f\n", i,
                   (double)c_re(a[i]), (double)c_im(a[i]), (double)c_re(b[i]),
                   (double)c_im(b[i]));
    }
...
static double impulse0(dofft_closure *k, int n, int vecn, C *inA, C *inB,
                       C *inC, C *outA, C *outB, C *outC, C *tmp, int rounds,
                       double tol) {
  int N = n * vecn;
  double e = 0.0;
  int j;
...
double impulse(dofft_closure *k, int n, int vecn, C *inA, C *inB, C *inC,
               C *outA, C *outB, C *outC, C *tmp, int rounds, double tol) {
  int i, j;
  double e = 0.0;
  ...
  e = dmax(e, impulse0(k, n, vecn, inA, inB, inC, outA, outB, outC, tmp, rounds,
                       tol));
...
double linear(dofft_closure *k, int realp, int n, C *inA, C *inB, C *inC,
              C *outA, C *outB, C *outC, C *tmp, int rounds, double tol) {
  int j;
  double e = 0.0;
...
double tf_shift(dofft_closure *k, int realp, const bench_tensor *sz, int n,
                int vecn, double sign, C *inA, C *inB, C *outA, C *outB, C *tmp,
  ...
  for (dim = 0; dim < sz->rnk; ++dim) {
    ...
    for (j = 0; j < rounds; ++j) {
      ...
      if (which_shift == TIME_SHIFT) {
        ...
        k->apply(k, inB, outB);
        for (i = 0; i < vecn; ++i)
          aphase_shift(tmp + i * n, outB + i * n, ncur, nb, na, sign);
        e = dmax(e, acmp(tmp, outA, N, "time shift", tol));
      } else {
        for (i = 0; i < vecn; ++i) {
          if (realp)
            mkhermitian(inA + i * n, sz->rnk, sz->dims, 1);
          aphase_shift(inB + i * n, inA + i * n, ncur, nb, na, -sign);
        }
        k->apply(k, inA, outA);
        k->apply(k, inB, outB);
        for (i = 0; i < vecn; ++i)
...
void preserves_input(dofft_closure *k, aconstrain constrain, int n, C *inA,
                     C *inB, C *outB, int rounds) {
  int j;
...
static int one_accuracy_test(dofft_closure *k, aconstrain constrain, int sign,
                             int n, C *a, C *b, double t[6]) {
  double err[6];
...
void accuracy_test(dofft_closure *k, aconstrain constrain, int sign, int n,
                   C *a, C *b, int rounds, int impulse_rounds, double t[6]) {
  int r, i;
  int ntests = 0;
