...
static void cpy0(dotens2_closure *k_, int indxa, int ondxa, int indxb,
                 int ondxb) {
...
static void cpy(R *ra, R *ia, const bench_tensor *sza, int scalea, R *rb, R *ib,
                const bench_tensor *szb, int scaleb) {
  cpy_closure k;
  k.k.apply = cpy0;
  k.ra = ra;
  k.ia = ia;
...
static void dft_apply(dofft_closure *k_, bench_complex *in,
                      bench_complex *out) {
  ...
  cpy(&c_re(in[0]), &c_im(in[0]), pckdsz, 1, ri, ii, totalsz, totalscale);
  after_problem_ccopy_from(p, ri, ii);
  doit(1, p);
  after_problem_ccopy_to(p, ro, io);
  if (k->k.recopy_input)
    cpy(ri, ii, totalsz_swap, totalscale, &c_re(in[0]), &c_im(in[0]),
        pckdsz_swap, 1);
  cpy(ro, io, totalsz, totalscale, &c_re(out[0]), &c_im(out[0]), pckdsz, 1);
...
void verify_dft(bench_problem *p, int rounds, double tol, errors *e) {
  ...
  e->l = linear(&k.k, 0, N, inA, inB, inC, outA, outB, outC, tmp, rounds, tol);

  e->s = 0.0;
  e->s = dmax(e->s, tf_shift(&k.k, 0, p->sz, n, vecn, p->sign, inA, inB, outA,
