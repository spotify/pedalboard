...
void problem_alloc(bench_problem *p) {
  ...
  if (p->kind == PROBLEM_COMPLEX) {
    ...
    if (p->in_place) {
      ...
      p->ophyssz = osz;
      p->outphys = out =
          (bench_complex *)bench_malloc(osz * sizeof(bench_complex));
      p->out = out - olb;
    }
    ...
    p->iphyssz = isz;
    p->inphys = in =
        (bench_real *)bench_malloc(p->iphyssz * sizeof(bench_real));
    p->in = in - ilb;
    ...
    p->ophyssz = osz;
    p->outphys = out =
        (bench_real *)bench_malloc(p->ophyssz * sizeof(bench_real));
    p->out = out - olb;
    ...
    if (p->in_place) {
      ...
      p->iphyssz = isz;
      p->inphys = in =
          (bench_complex *)bench_malloc(isz * sizeof(bench_complex));
      p->in = in - ilb;
    }
  } else {
    BENCH_ASSERT(0); /* TODO */
