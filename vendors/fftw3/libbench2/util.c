...
#endif
double bench_drand(void) { return drand48() - 0.5; }
#if defined(HAVE_DECL_SRAND48) && !HAVE_DECL_SRAND48
extern void srand48(long);
#endif
void bench_srand(int seed) { srand48(seed); }
#else
double bench_drand(void) {
  double d = rand();
  return (d / (double)RAND_MAX) - 0.5;
}
void bench_srand(int seed) { srand(seed); }
#endif
...
static int bench_malloc_total = 0, bench_malloc_max = 0,
           bench_malloc_cnt_max = 0;
...
void bench_free(void *p) {
...

  {
    ...
    BENCH_ASSERT((bench_malloc_cnt == 0 && bench_malloc_total == 0) ||
...
void *bench_malloc(size_t n) {
     ...
     sizeof(void*)-aligned.  Assumes size_t = uintptr_t.  */
  {
    ...
    if ((p0 = malloc(n + MIN_ALIGNMENT))) {
      p = (void *)(((size_t)p0 + MIN_ALIGNMENT) &
