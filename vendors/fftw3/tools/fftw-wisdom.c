...
static void add_problem(const char *pstring, bench_problem ***p, int *ip,
                        int *np) {
  if (*ip >= *np) {
    *np = *np * 2 + 1;
...
static void help(FILE *f, const char *program_name) {
  fprintf(
      ...
      "    Create wisdom (pre-planned/optimized transforms) for specified "
      "sizes,\n"
...
static char canonical_sizes[][32] = {
    ...
    "2x2",       "4x4",        "8x8",     "10x10",    "16x16",    "32x32",
    "64x64",     "100x100",    "128x128", "256x256",  "512x512",  "1000x1000",
    "1024x1024",
...
int bench_main(int argc, char *argv[]) {
  ...
  while ((c = my_getopt(argc, argv, options)) != -1) {
    ...
    case 'o':
      ...
      if (!strcmp(my_optarg, "-"))
        ...
        output_fname =
            (char *)bench_malloc(sizeof(char) * (strlen(my_optarg) + 1));
    ...
    case 'w': {
      ...
      if (strcmp(my_optarg, "-") && !(w = fopen(my_optarg, "r"))) {
        fprintf(stderr, "fftw-wisdom: error opening \"%s\": ", my_optarg);
        perror("");
        exit(EXIT_FAILURE);
      }
      if (!FFTW(import_wisdom_from_file)(w)) {
        fprintf(stderr,
                "fftw_wisdom: error reading wisdom "
                "from \"%s\"\n",
                my_optarg);
        exit(EXIT_FAILURE);
      }
  ...
  if (canonical) {
    for (i = 0; i < NELEM(canonical_sizes); ++i) {
      unsigned j;
      char types[][8] = {"cof", "cob", "cif", "cib",
                         "rof", "rob", "rif", "rib"};

      for (j = 0; j < NELEM(types); ++j) {
        char ps[64];
        if (!strchr(canonical_sizes[i], 'x') || !strchr(types[j], 'o')) {
#ifdef HAVE_SNPRINTF
  ...
  if (!output_fname)
    output_file = stdout;
  else if (!(output_file = fopen(output_fname, "w"))) {
    fprintf(stderr, "fftw-wisdom: error creating \"%s\"", output_fname);
    perror("");
    exit(EXIT_FAILURE);
  }
  ...
  for (iproblem = 0; iproblem < nproblems; ++iproblem) {
    if (hours <= 0 || hours > (time((time_t *)0) - begin) / 3600.0)
      do_problem(problems[iproblem]);
    problem_destroy(problems[iproblem]);
  }
  free(problems);

  if (verbose && hours > 0 && hours < (time((time_t *)0) - begin) / 3600.0)
