...
static void awake(plan *ego_, enum wakefulness wakefulness) {
  ...
  default:
    ...
    ego->omega = mkomega(wakefulness, ego->cld_omega, ego->n, ego->ginv);
    break;
  }
}
...
static void print(const plan *ego_, printer *p) {
  const P *ego = (const P *)ego_;
  p->print(p, "(dft-rader-%D%ois=%oos=%(%p%)", ego->n, ego->is, ego->os,
           ego->cld1);
...
static int applicable(const solver *ego_, const problem *p_,
  ...
  UNUSED(ego_);
  return (1 && p->sz->rnk == 1 && p->vecsz->rnk == 0 &&
...
static int mkP(P *pln, INT n, INT is, INT os, R *ro, R *io, planner *plnr) {
  plan *cld1 = (plan *)0;
  plan *cld2 = (plan *)0;
...
void X(dft_rader_register)(planner *p) { REGISTER_SOLVER(p, mksolver()); }
