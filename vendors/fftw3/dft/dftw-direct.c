...
static void apply(const plan *ego_, R *rio, R *iio) {
  ...
  for (i = 0; i < ego->v; ++i, rio += ego->vs, iio += ego->vs) {
    ...
    ego->k(rio + mb * ms, iio + mb * ms, ego->td->W, ego->rs, mb, ego->me, ms);
  }
}
...
static void apply_extra_iter(const plan *ego_, R *rio, R *iio) {
  ...
  for (i = 0; i < v; ++i, rio += vs, iio += vs) {
    ego->k(rio + mb * ms, iio + mb * ms, ego->td->W, ego->rs, mb, mm, ms);
    ego->k(rio + mm * ms, iio + mm * ms, ego->td->W, ego->rs, mm, mm + 2, 0);
  }
}
...
static void dobatch(const P *ego, R *rA, R *iA, INT mb, INT me, R *buf) {
  ...
  X(cpy2d_pair_ci)
  (rA + mb * ms, iA + mb * ms, buf, buf + 1, ego->r, rs, brs, me - mb, ms, 2);
  ego->k(buf, buf + 1, ego->td->W, ego->brs, mb, me, 2);
  X(cpy2d_pair_co)
  (buf, buf + 1, rA + mb * ms, iA + mb * ms, ego->r, brs, rs, me - mb, 2, ms);
}
...
static void print(const plan *ego_, printer *p) {
  ...
  if (slv->bufferedp)
    p->print(p, "(dftw-directbuf/%D-%D/%D%v \"%s\")", compute_batchsize(ego->r),
             ego->r, X(twiddle_length)(ego->r, e->tw), ego->v, e->nam);
  else
    p->print(p, "(dftw-direct-%D/%D%v \"%s\")", ego->r,
             X(twiddle_length)(ego->r, e->tw), ego->v, e->nam);
}

static int applicable0(const S *ego, INT r, INT irs, INT ors, INT m, INT ms,
                       INT v, INT ivs, INT ovs, INT mb, INT me, R *rio, R *iio,
                       const planner *plnr, INT *extra_iter) {
  const ct_desc *e = ego->desc;
  UNUSED(v);

  return (
      1 && r == e->radix && irs == ors /* in-place along R */
          ...
          (*extra_iter = 1,
           (1
            /* FIXME: require full array, otherwise some threads
               ...
               this case */
            && mb == 0 && me == m &&
            e->genus->okp(e, rio, iio, irs, ivs, m, mb, me - 1, ms, plnr) &&
            e->genus->okp(e, rio, iio, irs, ivs, m, me - 1, me + 1, ms, plnr))))

      && (e->genus->okp(e, rio + ivs, iio + ivs, irs, ivs, m, mb,
                        me - *extra_iter, ms, plnr))

  );
}

static int applicable0_buf(const S *ego, INT r, INT irs, INT ors, INT m, INT ms,
                           INT v, INT ivs, INT ovs, INT mb, INT me, R *rio,
                           R *iio, const planner *plnr) {
  const ct_desc *e = ego->desc;
  INT batchsz;
  UNUSED(v);
  UNUSED(ms);
  ...
  return (1 && r == e->radix && irs == ors /* in-place along R */
          && ivs == ovs                    /* in-place along V */

          /* check for alignment/vector length restrictions, both for
             batchsize and for the remainder */
          && (batchsz = compute_batchsize(r), 1) &&
          (e->genus->okp(e, 0, ((const R *)0) + 1, 2 * batchsz, 0, m, mb,
                         mb + batchsz, 2, plnr)) &&
          (e->genus->okp(e, 0, ((const R *)0) + 1, 2 * batchsz, 0, m, mb, me, 2,
                         plnr)));
}

static int applicable(const S *ego, INT r, INT irs, INT ors, INT m, INT ms,
                      INT v, INT ivs, INT ovs, INT mb, INT me, R *rio, R *iio,
                      const planner *plnr, INT *extra_iter) {
  if (ego->bufferedp) {
    *extra_iter = 0;
    if (!applicable0_buf(ego, r, irs, ors, m, ms, v, ivs, ovs, mb, me, rio, iio,
                         plnr))
      return 0;
  } else {
    if (!applicable0(ego, r, irs, ors, m, ms, v, ivs, ovs, mb, me, rio, iio,
                     plnr, extra_iter))
      return 0;
  }
...
static plan *mkcldw(const ct_solver *ego_, INT r, INT irs, INT ors, INT m,
                    INT ms, INT v, INT ivs, INT ovs, INT mstart, INT mcount,
  ...
  if (!applicable(ego, r, irs, ors, m, ms, v, ivs, ovs, mstart, mstart + mcount,
...
static void regone(planner *plnr, kdftw codelet, const ct_desc *desc, int dec,
                   int bufferedp) {
  ...
  if (X(mksolver_ct_hook)) {
    slv = (S *)X(mksolver_ct_hook)(sizeof(S), desc->radix, dec, mkcldw, 0);
    slv->k = codelet;
    slv->desc = desc;
    slv->bufferedp = bufferedp;
    REGISTER_SOLVER(plnr, &(slv->super.super));
...
void X(regsolver_ct_directw)(planner *plnr, kdftw codelet, const ct_desc *desc,
                             int dec) {
