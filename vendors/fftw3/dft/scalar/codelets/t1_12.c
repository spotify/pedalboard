 ...
 * -pipeline-latency 4 -n 12 -name t1_12 -include dft/scalar/t.h */
...
static void t1_12(R *ri, R *ii, const R *W, stride rs, INT mb, INT me, INT ms) {
  ...
  {
    INT m;
    for (m = mb, W = W + (mb * 22); m < me; m = m + 1, ri = ri + ms,
        ii = ii + ms, W = W + 22, MAKE_VOLATILE_STRIDE(24, rs)) {
      ...
      {
        E T1m, T1K, T2p, T2y, T2s, T2x, T1t, T1L, T1B, T1N, T1W, T25, T22, T26,
            T1I;
        E T1O;
        {
          E T1g, T2n, T2q, T1n;
...
static const tw_instr twinstr[] = {{TW_FULL, 0, 12}, {TW_NEXT, 1, 0}};

static const ct_desc desc = {12, "t1_12", twinstr, &GENUS, {72, 22, 46, 0},
                             0,  0,       0};

void X(codelet_t1_12)(planner *p) { X(kdft_dit_register)(p, t1_12, &desc); }
#else
...
static const ct_desc desc = {12, "t1_12", twinstr, &GENUS, {88, 30, 30, 0},
                             0,  0,       0};
