 ...
 * -pipeline-latency 4 -n 10 -name t1_10 -include dft/scalar/t.h */
...
static void t1_10(R *ri, R *ii, const R *W, stride rs, INT mb, INT me, INT ms) {
  ...
  {
    INT m;
    for (m = mb, W = W + (mb * 18); m < me; m = m + 1, ri = ri + ms,
        ii = ii + ms, W = W + 18, MAKE_VOLATILE_STRIDE(20, rs)) {
...
static const tw_instr twinstr[] = {{TW_FULL, 0, 10}, {TW_NEXT, 1, 0}};

static const ct_desc desc = {10, "t1_10", twinstr, &GENUS, {48, 18, 54, 0},
                             0,  0,       0};

void X(codelet_t1_10)(planner *p) { X(kdft_dit_register)(p, t1_10, &desc); }
#else
...
static const ct_desc desc = {10, "t1_10", twinstr, &GENUS, {72, 30, 30, 0},
                             0,  0,       0};
