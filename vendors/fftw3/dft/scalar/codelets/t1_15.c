 ...
 * -pipeline-latency 4 -n 15 -name t1_15 -include dft/scalar/t.h */
...
static void t1_15(R *ri, R *ii, const R *W, stride rs, INT mb, INT me, INT ms) {
  ...
  {
    INT m;
    for (m = mb, W = W + (mb * 28); m < me; m = m + 1, ri = ri + ms,
        ii = ii + ms, W = W + 28, MAKE_VOLATILE_STRIDE(30, rs)) {
      E T1, T3j, T1G, T3u, Te, T1B, T3i, T3t, T1y, T2i, T2a, T2M, T37, T2V, Tz;
      E T2e, T1O, T2t, T39, T2X, TT, T2f, T1V, T2z, T3a, T2Y, T1e, T2h, T23,
          T2G;
      E T36, T2U;
      {
        E T7, T1D, Td, T1F;
...
static const tw_instr twinstr[] = {{TW_FULL, 0, 15}, {TW_NEXT, 1, 0}};

static const ct_desc desc = {15, "t1_15", twinstr, &GENUS, {72, 28, 112, 0},
                             0,  0,       0};

void X(codelet_t1_15)(planner *p) { X(kdft_dit_register)(p, t1_15, &desc); }
#else
...
static void t1_15(R *ri, R *ii, const R *W, stride rs, INT mb, INT me, INT ms) {
  ...
  DK(KP866025403, +0.866025403784438646763723170752936183471402627);
  {
    ...
    for (m = mb, W = W + (mb * 28); m < me; m = m + 1, ri = ri + ms,
        ii = ii + ms, W = W + 28, MAKE_VOLATILE_STRIDE(30, rs)) {
      E T1q, T34, Td, T1n, T2S, T35, T13, T1k, T1l, T2E, T2F, T2O, T1H, T1T,
          T2k;
      ...
      {
        E TR, T2c, T18, T2h, TW, T1E, T11, T1F, T12, T2d, T1d, T1J, T1i, T1K,
            T1j;
        E T2i;
        {
          E TO, TQ, TN, TP;
...
static const ct_desc desc = {15, "t1_15", twinstr, &GENUS, {128, 56, 56, 0},
                             0,  0,       0};
