 ...
 * -pipeline-latency 4 -n 16 -name t1_16 -include dft/scalar/t.h */
...
static void t1_16(R *ri, R *ii, const R *W, stride rs, INT mb, INT me, INT ms) {
  ...
  {
    INT m;
    for (m = mb, W = W + (mb * 30); m < me; m = m + 1, ri = ri + ms,
        ii = ii + ms, W = W + 30, MAKE_VOLATILE_STRIDE(32, rs)) {
      ...
      {
        E T1O, T3B, T3H, T2E, T23, T3C, T2O, T2S, T2H, T3I, T2j, T2B, T2L, T2R,
            T2y;
        E T2C;
        {
          E T1V, T22, T2b, T2i;
...
static const tw_instr twinstr[] = {{TW_FULL, 0, 16}, {TW_NEXT, 1, 0}};

static const ct_desc desc = {16, "t1_16", twinstr, &GENUS, {104, 30, 70, 0},
                             0,  0,       0};

void X(codelet_t1_16)(planner *p) { X(kdft_dit_register)(p, t1_16, &desc); }
#else
...
static void t1_16(R *ri, R *ii, const R *W, stride rs, INT mb, INT me, INT ms) {
  ...
  DK(KP707106781, +0.707106781186547524400844362104849039284835938);
  {
    ...
    for (m = mb, W = W + (mb * 30); m < me; m = m + 1, ri = ri + ms,
      ...
      E T2d, T1f, T1q, T2B, T2C, T2D, T2E, T1Z, T2j, T24, T2k, TS, T13, T2w,
          T2x;
...
static const ct_desc desc = {16, "t1_16", twinstr, &GENUS, {136, 46, 38, 0},
                             0,  0,       0};
