 ...
 * -pipeline-latency 4 -n 4 -name t1_4 -include dft/scalar/t.h */
...
static void t1_4(R *ri, R *ii, const R *W, stride rs, INT mb, INT me, INT ms) {
  {
    ...
    for (m = mb, W = W + (mb * 6); m < me; m = m + 1, ri = ri + ms,
        ii = ii + ms, W = W + 6, MAKE_VOLATILE_STRIDE(8, rs)) {
...
static const ct_desc desc = {4, "t1_4", twinstr, &GENUS, {16, 6, 6, 0},
                             0, 0,      0};

void X(codelet_t1_4)(planner *p) { X(kdft_dit_register)(p, t1_4, &desc); }
#else
