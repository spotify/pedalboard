 ...
 * -pipeline-latency 4 -twiddle-log3 -precompute-twiddles -n 4 -name t2_4
...
static void t2_4(R *ri, R *ii, const R *W, stride rs, INT mb, INT me, INT ms) {
  {
    ...
    for (m = mb, W = W + (mb * 4); m < me; m = m + 1, ri = ri + ms,
        ii = ii + ms, W = W + 4, MAKE_VOLATILE_STRIDE(8, rs)) {
...
static const ct_desc desc = {4, "t2_4", twinstr, &GENUS, {16, 8, 8, 0},
                             0, 0,      0};

void X(codelet_t2_4)(planner *p) { X(kdft_dit_register)(p, t2_4, &desc); }
#else
