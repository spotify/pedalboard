 ...
 * -pipeline-latency 4 -n 8 -name t1_8 -include dft/scalar/t.h */
...
static void t1_8(R *ri, R *ii, const R *W, stride rs, INT mb, INT me, INT ms) {
  ...
  {
    INT m;
    for (m = mb, W = W + (mb * 14); m < me; m = m + 1, ri = ri + ms,
        ii = ii + ms, W = W + 14, MAKE_VOLATILE_STRIDE(16, rs)) {
...
static const ct_desc desc = {8, "t1_8", twinstr, &GENUS, {44, 14, 22, 0},
                             0, 0,      0};

void X(codelet_t1_8)(planner *p) { X(kdft_dit_register)(p, t1_8, &desc); }
#else
...
static const ct_desc desc = {8, "t1_8", twinstr, &GENUS, {52, 18, 14, 0},
                             0, 0,      0};
