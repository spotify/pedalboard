 ...
 * -pipeline-latency 4 -n 2 -name t1_2 -include dft/scalar/t.h */
...
static void t1_2(R *ri, R *ii, const R *W, stride rs, INT mb, INT me, INT ms) {
  {
    INT m;
    for (m = mb, W = W + (mb * 2); m < me; m = m + 1, ri = ri + ms,
        ii = ii + ms, W = W + 2, MAKE_VOLATILE_STRIDE(4, rs)) {
...
void X(codelet_t1_2)(planner *p) { X(kdft_dit_register)(p, t1_2, &desc); }
#else
