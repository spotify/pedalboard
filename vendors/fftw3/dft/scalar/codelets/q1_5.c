 ...
 * -pipeline-latency 4 -reload-twiddle -dif -n 5 -name q1_5 -include
 * dft/scalar/q.h */
...
static void q1_5(R *rio, R *iio, const R *W, stride rs, stride vs, INT mb,
                 INT me, INT ms) {
  ...
  {
    INT m;
    for (m = mb, W = W + (mb * 8); m < me; m = m + 1, rio = rio + ms,
        iio = iio + ms, W = W + 8, MAKE_VOLATILE_STRIDE(10, rs),
        MAKE_VOLATILE_STRIDE(0, vs)) {
      E T1, Tb, TM, Tw, T8, Ta, Tn, Tj, TH, Ts, Tq, Tr, TV, T15, T1G;
      E T1q, T12, T14, T1h, T1d, T1B, T1m, T1k, T1l, T1P, T1Z, T2A, T2k, T1W,
          T1Y;
      E T2b, T27, T2v, T2g, T2e, T2f, T3Z, T3V, T4j, T44, T42, T43, T3D, T3N,
          T4o;
      E T48, T3K, T3M, T2J, T2T, T3u, T3e, T2Q, T2S, T35, T31, T3p, T3a, T38,
          T39;
      {
        E T7, Tv, T4, Tu;
        T1 = rio[0];
...
static const tw_instr twinstr[] = {{TW_FULL, 0, 5}, {TW_NEXT, 1, 0}};

static const ct_desc desc = {5, "q1_5", twinstr, &GENUS, {70, 40, 130, 0},
                             0, 0,      0};

void X(codelet_q1_5)(planner *p) { X(kdft_difsq_register)(p, q1_5, &desc); }
#else
...
static void q1_5(R *rio, R *iio, const R *W, stride rs, stride vs, INT mb,
  ...
  DK(KP559016994, +0.559016994374947424102293417182819058860154590);
  {
    ...
    for (m = mb, W = W + (mb * 8); m < me; m = m + 1, rio = rio + ms,
      ...
      E T1X, T1R, T2b, T1W, T1Y, T1Z, T3v, T3p, T3J, T3u, T3w, T3x, T37, T3g,
          T3M;
      E T3B, T3e, T3h, T2l, T2u, T30, T2P, T2s, T2v, T2J, T2D, T2X, T2I, T2K,
          T2L;
      {
        E T7, Tu, T4, Tt;
        T1 = rio[0];
...
static const ct_desc desc = {5, "q1_5", twinstr, &GENUS, {130, 70, 70, 0},
                             0, 0,      0};
