 ...
 * -pipeline-latency 4 -n 9 -name t1_9 -include dft/scalar/t.h */
...
static void t1_9(R *ri, R *ii, const R *W, stride rs, INT mb, INT me, INT ms) {
  ...
  {
    INT m;
    for (m = mb, W = W + (mb * 16); m < me; m = m + 1, ri = ri + ms,
        ii = ii + ms, W = W + 16, MAKE_VOLATILE_STRIDE(18, rs)) {
      ...
      {
        E T11, T1z, T1X, T21, T1f, T1w, T1t, T1x, T1u, T1Y, T1C, T1I, T1F, T1J,
            T1G;
        E T22, TV, T1V;
...
static const tw_instr twinstr[] = {{TW_FULL, 0, 9}, {TW_NEXT, 1, 0}};

static const ct_desc desc = {9, "t1_9", twinstr, &GENUS, {24, 16, 72, 0},
                             0, 0,      0};

void X(codelet_t1_9)(planner *p) { X(kdft_dit_register)(p, t1_9, &desc); }
#else
...
static const ct_desc desc = {9, "t1_9", twinstr, &GENUS, {60, 36, 36, 0},
                             0, 0,      0};
