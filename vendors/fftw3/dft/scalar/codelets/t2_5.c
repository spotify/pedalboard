 ...
 * -pipeline-latency 4 -twiddle-log3 -precompute-twiddles -n 5 -name t2_5
...
static void t2_5(R *ri, R *ii, const R *W, stride rs, INT mb, INT me, INT ms) {
  ...
  {
    INT m;
    for (m = mb, W = W + (mb * 4); m < me; m = m + 1, ri = ri + ms,
        ii = ii + ms, W = W + 4, MAKE_VOLATILE_STRIDE(10, rs)) {
...
static const tw_instr twinstr[] = {
    {TW_CEXP, 0, 1}, {TW_CEXP, 0, 3}, {TW_NEXT, 1, 0}};

static const ct_desc desc = {5, "t2_5", twinstr, &GENUS, {14, 10, 30, 0},
                             0, 0,      0};

void X(codelet_t2_5)(planner *p) { X(kdft_dit_register)(p, t2_5, &desc); }
#else
...
static const ct_desc desc = {5, "t2_5", twinstr, &GENUS, {30, 18, 14, 0},
                             0, 0,      0};
