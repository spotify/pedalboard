 ...
 * -pipeline-latency 4 -n 7 -name t1_7 -include dft/scalar/t.h */
...
static void t1_7(R *ri, R *ii, const R *W, stride rs, INT mb, INT me, INT ms) {
  ...
  {
    INT m;
    for (m = mb, W = W + (mb * 12); m < me; m = m + 1, ri = ri + ms,
        ii = ii + ms, W = W + 12, MAKE_VOLATILE_STRIDE(14, rs)) {
...
static const tw_instr twinstr[] = {{TW_FULL, 0, 7}, {TW_NEXT, 1, 0}};

static const ct_desc desc = {7, "t1_7", twinstr, &GENUS, {18, 12, 54, 0},
                             0, 0,      0};

void X(codelet_t1_7)(planner *p) { X(kdft_dit_register)(p, t1_7, &desc); }
#else
  ...
  DK(KP974927912, +0.974927912181823607018131682993931217232785801);
  {
    ...
    for (m = mb, W = W + (mb * 12); m < me; m = m + 1, ri = ri + ms,
      ...
      }
      {
        ...
        T10 =
            FMA(KP623489801, TP, TR) + FNMA(KP222520933, TQ, KP900968867 * TO);
        ii[WS(rs, 3)] = TZ + T10;
        ii[WS(rs, 4)] = T10 - TZ;
      }
    }
...
static const ct_desc desc = {7, "t1_7", twinstr, &GENUS, {36, 24, 36, 0},
                             0, 0,      0};
