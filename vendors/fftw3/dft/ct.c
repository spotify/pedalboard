...
ct_solver *(*X(mksolver_ct_hook))(size_t, INT, int, ct_mkinferior,
                                  ct_force_vrecursion) = 0;
...
static void print(const plan *ego_, printer *p) {
  ...
  p->print(p, "(dft-ct-%s/%D%(%p%)%(%p%))",
           ego->super.apply == apply_dit ? "dit" : "dif", ego->r, ego->cldw,
           ego->cld);
}
...
static int applicable0(const ct_solver *ego, const problem *p_, planner *plnr) {
  ...
  return (1 && p->sz->rnk == 1 &&
          ...
          && (ego->dec == DECDIT || p->ri == p->ro || !NO_DESTROY_INPUTP(plnr))
...
int X(ct_applicable)(const ct_solver *ego, const problem *p_, planner *plnr) {
  ...
  return (0 || ego->dec == DECDIF + TRANSPOSE || p->vecsz->rnk == 0 ||
          !NO_VRECURSEP(plnr) ||
...
static plan *mkplan(const solver *ego_, const problem *p_, planner *plnr) {
  ...
  switch (ego->dec) {
  case DECDIT: {
    cldw = ego->mkcldw(ego, r, m * d[0].os, m * d[0].os, m, d[0].os, v, ovs,
                       ovs, 0, m, p->ro, p->io, plnr);
    if (!cldw)
      goto nada;

    cld = X(mkplan_d)(
        plnr,
        X(mkproblem_dft_d)(X(mktensor_1d)(m, r * d[0].is, d[0].os),
                           X(mktensor_2d)(r, d[0].is, m * d[0].os, v, ivs, ovs),
                           p->ri, p->ii, p->ro, p->io));
    if (!cld)
      goto nada;
  ...
  case DECDIF + TRANSPOSE: {
    ...
    if (ego->dec == DECDIF + TRANSPOSE) {
      ...
      if (!(1 && r == v && d[0].is == r * cors))
        goto nada;

      /* FIXME: allow in-place only for now, like in
         fftw-3.[01] */
      if (!(1 && p->ri == p->ro && d[0].is == r * d[0].os && cors == d[0].os &&
            covs == ovs))
        goto nada;
    } else {
      cors = m * d[0].is;
    ...
    cldw = ego->mkcldw(ego, r, m * d[0].is, cors, m, d[0].is, v, ivs, covs, 0,
                       m, p->ri, p->ii, plnr);
    if (!cldw)
      goto nada;

    cld = X(mkplan_d)(
        plnr, X(mkproblem_dft_d)(X(mktensor_1d)(m, d[0].is, r * d[0].os),
                                 X(mktensor_2d)(r, cors, d[0].os, v, covs, ovs),
                                 p->ri, p->ii, p->ro, p->io));
    if (!cld)
      goto nada;
