 ...
 * -variables 4 -pipeline-latency 8 -n 9 -name t1fuv_9 -include dft/simd/t1fu.h
 */
...
static void t1fuv_9(R *ri, R *ii, const R *W, stride rs, INT mb, INT me,
                    INT ms) {
  ...
  DVK(KP500000000, +0.500000000000000000000000000000000000000000000);
  {
    ...
    R *x;
    x = ri;
    for (m = mb, W = W + (mb * ((TWVL / VL) * 16)); m < me; m = m + VL,
        x = x + (VL * ms), W = W + (TWVL * 16), MAKE_VOLATILE_STRIDE(9, rs)) {
...
static const tw_instr twinstr[] = {VTW(0, 1), VTW(0, 2), VTW(0, 3),
                                   ...
                                   VTW(0, 7), VTW(0, 8), {TW_NEXT, VL, 0}};

static const ct_desc desc = {
    9, XSIMD_STRING("t1fuv_9"), twinstr, &GENUS, {20, 20, 34, 0}, 0, 0, 0};
 ...
 * 4 -pipeline-latency 8 -n 9 -name t1fuv_9 -include dft/simd/t1fu.h */
  ...
  DVK(KP866025403, +0.866025403784438646763723170752936183471402627);
  {
    ...
    for (m = mb, W = W + (mb * ((TWVL / VL) * 16)); m < me; m = m + VL,
      ...
      }
      {
        ...
        TI = VBYI(VSUB(VFNMS(LDK(KP342020143), Tv,
                             VFNMS(LDK(KP150383733), Tt,
                                   VFNMS(LDK(KP984807753), Ts,
                                         VMUL(LDK(KP813797681), Tw)))),
                       TA));
        ...
        Ty = VADD(Tu, Tx);
        Tr = VFNMS(LDK(KP500000000), T6, T1);
        TH = VFMA(LDK(KP173648177), Ts,
                  VFNMS(LDK(KP296198132), Tw,
                        VFNMS(LDK(KP939692620), Tv,
                              VFNMS(LDK(KP852868531), Tt, Tr))));
        ...
        ST(&(x[WS(rs, 2)]), VADD(TH, TI), ms, &(x[0]));
        {
          ...
          TF = VFMA(LDK(KP866025403), VSUB(TB, TC),
                    VFNMS(LDK(KP500000000), Ty, Tr));
          TG = VBYI(VADD(TA, VFNMS(LDK(KP500000000), TD,
                                   VMUL(LDK(KP866025403), VSUB(Tx, Tu)))));
...
static const ct_desc desc = {
    9, XSIMD_STRING("t1fuv_9"), twinstr, &GENUS, {38, 26, 16, 0}, 0, 0, 0};
