 ...
 * -no-generate-bytw -n 4 -name t3bv_4 -include dft/simd/t3b.h -sign 1 */
...
static void t3bv_4(R *ri, R *ii, const R *W, stride rs, INT mb, INT me,
                   INT ms) {
  {
    INT m;
    R *x;
    ...
    for (m = mb, W = W + (mb * ((TWVL / VL) * 4)); m < me; m = m + VL,
        x = x + (VL * ms), W = W + (TWVL * 4), MAKE_VOLATILE_STRIDE(4, rs)) {
      V T2, T3, T4;
      T2 = LDW(&(W[0]));
      T3 = LDW(&(W[TWVL * 2]));
      T4 = VZMULJ(T2, T3);
...
static const ct_desc desc = {
    4, XSIMD_STRING("t3bv_4"), twinstr, &GENUS, {10, 8, 2, 0}, 0, 0, 0};
 ...
 * 4 -name t3bv_4 -include dft/simd/t3b.h -sign 1 */
...
static const ct_desc desc = {
    4, XSIMD_STRING("t3bv_4"), twinstr, &GENUS, {12, 8, 0, 0}, 0, 0, 0};
