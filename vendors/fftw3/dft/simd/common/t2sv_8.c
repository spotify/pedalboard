 ...
 * -variables 4 -pipeline-latency 8 -twiddle-log3 -precompute-twiddles -n 8
 * -name t2sv_8 -include dft/simd/ts.h */
...
static void t2sv_8(R *ri, R *ii, const R *W, stride rs, INT mb, INT me,
                   INT ms) {
  DVK(KP707106781, +0.707106781186547524400844362104849039284835938);
  {
    INT m;
    for (m = mb, W = W + (mb * 6); m < me; m = m + (2 * VL),
        ri = ri + ((2 * VL) * ms), ii = ii + ((2 * VL) * ms),
        W = W + ((2 * VL) * 6), MAKE_VOLATILE_STRIDE(16, rs)) {
      ...
      }
      {
        ...
        }
        {
          ...
          ST(&(ri[WS(rs, 5)]), VFNMS(LDK(KP707106781), T1f, T10), ms,
             &(ri[WS(rs, 1)]));
          ST(&(ii[WS(rs, 5)]), VFNMS(LDK(KP707106781), T1A, T1z), ms,
             &(ii[WS(rs, 1)]));
          ST(&(ri[WS(rs, 1)]), VFMA(LDK(KP707106781), T1f, T10), ms,
             &(ri[WS(rs, 1)]));
          ST(&(ii[WS(rs, 1)]), VFMA(LDK(KP707106781), T1A, T1z), ms,
             &(ii[WS(rs, 1)]));
          ST(&(ri[WS(rs, 7)]), VFNMS(LDK(KP707106781), T1j, T1g), ms,
             &(ri[WS(rs, 1)]));
          ST(&(ii[WS(rs, 7)]), VFNMS(LDK(KP707106781), T1C, T1B), ms,
             &(ii[WS(rs, 1)]));
          ST(&(ri[WS(rs, 3)]), VFMA(LDK(KP707106781), T1j, T1g), ms,
             &(ri[WS(rs, 1)]));
          ST(&(ii[WS(rs, 3)]), VFMA(LDK(KP707106781), T1C, T1B), ms,
             &(ii[WS(rs, 1)]));
        }
        {
...
static const tw_instr twinstr[] = {
    VTW(0, 1), VTW(0, 3), VTW(0, 7), {TW_NEXT, (2 * VL), 0}};

static const ct_desc desc = {
    8, XSIMD_STRING("t2sv_8"), twinstr, &GENUS, {44, 20, 30, 0}, 0, 0, 0};
 ...
 * -pipeline-latency 8 -twiddle-log3 -precompute-twiddles -n 8 -name t2sv_8
 * -include dft/simd/ts.h */
...
static const ct_desc desc = {
    8, XSIMD_STRING("t2sv_8"), twinstr, &GENUS, {56, 26, 18, 0}, 0, 0, 0};
