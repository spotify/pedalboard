 ...
 * -variables 4 -pipeline-latency 8 -n 7 -name t1buv_7 -include dft/simd/t1bu.h
 * -sign 1 */
...
static void t1buv_7(R *ri, R *ii, const R *W, stride rs, INT mb, INT me,
                    INT ms) {
  ...
  DVK(KP356895867, +0.356895867892209443894399510021300583399127187);
  {
    ...
    R *x;
    x = ii;
    for (m = mb, W = W + (mb * ((TWVL / VL) * 12)); m < me; m = m + VL,
        x = x + (VL * ms), W = W + (TWVL * 12), MAKE_VOLATILE_STRIDE(7, rs)) {
...
static const ct_desc desc = {
    7, XSIMD_STRING("t1buv_7"), twinstr, &GENUS, {15, 15, 21, 0}, 0, 0, 0};
 ...
 * 4 -pipeline-latency 8 -n 7 -name t1buv_7 -include dft/simd/t1bu.h -sign 1 */
...
static void t1buv_7(R *ri, R *ii, const R *W, stride rs, INT mb, INT me,
  ...
  DVK(KP974927912, +0.974927912181823607018131682993931217232785801);
  {
    ...
    for (m = mb, W = W + (mb * ((TWVL / VL) * 12)); m < me; m = m + VL,
      ...
      To = VBYI(VFNMS(LDK(KP781831482), Ta,
                      VFNMS(LDK(KP433883739), Tf, VMUL(LDK(KP974927912), T5))));
      Tp = VFMA(LDK(KP623489801), Tj,
                VFNMS(LDK(KP900968867), Ti, VFNMS(LDK(KP222520933), Tk, Th)));
      ST(&(x[WS(rs, 2)]), VADD(To, Tp), ms, &(x[0]));
      ST(&(x[WS(rs, 5)]), VSUB(Tp, To), ms, &(x[WS(rs, 1)]));
      {
        V Tg, Tl, Tm, Tn;
        Tg =
            VBYI(VFMA(LDK(KP433883739), T5,
                      VFNMS(LDK(KP781831482), Tf, VMUL(LDK(KP974927912), Ta))));
        Tl = VFMA(LDK(KP623489801), Ti,
                  VFNMS(LDK(KP222520933), Tj, VFNMS(LDK(KP900968867), Tk, Th)));
        ST(&(x[WS(rs, 3)]), VADD(Tg, Tl), ms, &(x[WS(rs, 1)]));
        ST(&(x[WS(rs, 4)]), VSUB(Tl, Tg), ms, &(x[0]));
        Tm = VBYI(VFMA(LDK(KP781831482), T5,
                       VFMA(LDK(KP974927912), Tf, VMUL(LDK(KP433883739), Ta))));
        Tn = VFMA(LDK(KP623489801), Tk,
                  VFNMS(LDK(KP900968867), Tj, VFNMS(LDK(KP222520933), Ti, Th)));
...
static const ct_desc desc = {
    7, XSIMD_STRING("t1buv_7"), twinstr, &GENUS, {24, 18, 12, 0}, 0, 0, 0};
