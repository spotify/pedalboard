 ...
 * -variables 4 -pipeline-latency 8 -n 7 -name t1fuv_7 -include dft/simd/t1fu.h
 */
...
static void t1fuv_7(R *ri, R *ii, const R *W, stride rs, INT mb, INT me,
                    INT ms) {
...
static const ct_desc desc = {
    7, XSIMD_STRING("t1fuv_7"), twinstr, &GENUS, {15, 15, 21, 0}, 0, 0, 0};
 ...
 * 4 -pipeline-latency 8 -n 7 -name t1fuv_7 -include dft/simd/t1fu.h */
...
static void t1fuv_7(R *ri, R *ii, const R *W, stride rs, INT mb, INT me,
  ...
  DVK(KP433883739, +0.433883739117558120475768332848358754609990728);
  {
    ...
    for (m = mb, W = W + (mb * ((TWVL / VL) * 12)); m < me; m = m + VL,
      ...
      ST(&(x[0]), VADD(T1, VADD(T6, VADD(Tb, Tg))), ms, &(x[0]));
      Tp = VBYI(VFMA(LDK(KP433883739), Ti,
                     VFNMS(LDK(KP781831482), Tk, VMUL(LDK(KP974927912), Tj))));
      To = VFMA(LDK(KP623489801), Tb,
                VFNMS(LDK(KP222520933), Tg, VFNMS(LDK(KP900968867), T6, T1)));
      ST(&(x[WS(rs, 4)]), VSUB(To, Tp), ms, &(x[0]));
      ST(&(x[WS(rs, 3)]), VADD(To, Tp), ms, &(x[WS(rs, 1)]));
      {
        V Tl, Th, Tn, Tm;
        Tl = VBYI(
            VFNMS(LDK(KP781831482), Tj,
                  VFNMS(LDK(KP433883739), Tk, VMUL(LDK(KP974927912), Ti))));
        Th = VFMA(LDK(KP623489801), Tg,
                  VFNMS(LDK(KP900968867), Tb, VFNMS(LDK(KP222520933), T6, T1)));
        ST(&(x[WS(rs, 5)]), VSUB(Th, Tl), ms, &(x[WS(rs, 1)]));
        ST(&(x[WS(rs, 2)]), VADD(Th, Tl), ms, &(x[0]));
        Tn = VBYI(VFMA(LDK(KP781831482), Ti,
                       VFMA(LDK(KP974927912), Tk, VMUL(LDK(KP433883739), Tj))));
        Tm = VFMA(LDK(KP623489801), T6,
                  VFNMS(LDK(KP900968867), Tg, VFNMS(LDK(KP222520933), Tb, T1)));
...
static const ct_desc desc = {
    7, XSIMD_STRING("t1fuv_7"), twinstr, &GENUS, {24, 18, 12, 0}, 0, 0, 0};
