 ...
 * -variables 4 -pipeline-latency 8 -n 2 -name t1sv_2 -include dft/simd/ts.h */
...
static void t1sv_2(R *ri, R *ii, const R *W, stride rs, INT mb, INT me,
                   INT ms) {
  {
    INT m;
    for (m = mb, W = W + (mb * 2); m < me; m = m + (2 * VL),
        ri = ri + ((2 * VL) * ms), ii = ii + ((2 * VL) * ms),
        W = W + ((2 * VL) * 2), MAKE_VOLATILE_STRIDE(4, rs)) {
...
static const tw_instr twinstr[] = {VTW(0, 1), {TW_NEXT, (2 * VL), 0}};

static const ct_desc desc = {
    2, XSIMD_STRING("t1sv_2"), twinstr, &GENUS, {4, 2, 2, 0}, 0, 0, 0};
 ...
 * -pipeline-latency 8 -n 2 -name t1sv_2 -include dft/simd/ts.h */
