 ...
 * -variables 4 -pipeline-latency 8 -n 8 -name t1sv_8 -include dft/simd/ts.h */
...
static void t1sv_8(R *ri, R *ii, const R *W, stride rs, INT mb, INT me,
                   INT ms) {
  DVK(KP707106781, +0.707106781186547524400844362104849039284835938);
  {
    INT m;
    for (m = mb, W = W + (mb * 14); m < me; m = m + (2 * VL),
        ri = ri + ((2 * VL) * ms), ii = ii + ((2 * VL) * ms),
        W = W + ((2 * VL) * 14), MAKE_VOLATILE_STRIDE(16, rs)) {
      ...
      }
      {
        ...
        ST(&(ri[WS(rs, 5)]), VFNMS(LDK(KP707106781), T19, TU), ms,
           &(ri[WS(rs, 1)]));
        ST(&(ii[WS(rs, 5)]), VFNMS(LDK(KP707106781), T1u, T1t), ms,
           &(ii[WS(rs, 1)]));
        ST(&(ri[WS(rs, 1)]), VFMA(LDK(KP707106781), T19, TU), ms,
           &(ri[WS(rs, 1)]));
        ST(&(ii[WS(rs, 1)]), VFMA(LDK(KP707106781), T1u, T1t), ms,
           &(ii[WS(rs, 1)]));
        ST(&(ri[WS(rs, 7)]), VFNMS(LDK(KP707106781), T1d, T1a), ms,
           &(ri[WS(rs, 1)]));
        ST(&(ii[WS(rs, 7)]), VFNMS(LDK(KP707106781), T1w, T1v), ms,
           &(ii[WS(rs, 1)]));
        ST(&(ri[WS(rs, 3)]), VFMA(LDK(KP707106781), T1d, T1a), ms,
           &(ri[WS(rs, 1)]));
        ST(&(ii[WS(rs, 3)]), VFMA(LDK(KP707106781), T1w, T1v), ms,
           &(ii[WS(rs, 1)]));
      }
      {
...
static const tw_instr twinstr[] = {
    ...
    VTW(0, 5), VTW(0, 6), VTW(0, 7), {TW_NEXT, (2 * VL), 0}};

static const ct_desc desc = {
    8, XSIMD_STRING("t1sv_8"), twinstr, &GENUS, {44, 14, 22, 0}, 0, 0, 0};
 ...
 * -pipeline-latency 8 -n 8 -name t1sv_8 -include dft/simd/ts.h */
...
static const ct_desc desc = {
    8, XSIMD_STRING("t1sv_8"), twinstr, &GENUS, {52, 18, 14, 0}, 0, 0, 0};
