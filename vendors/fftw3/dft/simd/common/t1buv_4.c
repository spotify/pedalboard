 ...
 * -variables 4 -pipeline-latency 8 -n 4 -name t1buv_4 -include dft/simd/t1bu.h
 * -sign 1 */
...
static void t1buv_4(R *ri, R *ii, const R *W, stride rs, INT mb, INT me,
                    INT ms) {
  {
    INT m;
    R *x;
    ...
    for (m = mb, W = W + (mb * ((TWVL / VL) * 6)); m < me; m = m + VL,
        x = x + (VL * ms), W = W + (TWVL * 6), MAKE_VOLATILE_STRIDE(4, rs)) {
...
static const ct_desc desc = {
    4, XSIMD_STRING("t1buv_4"), twinstr, &GENUS, {9, 6, 2, 0}, 0, 0, 0};
 ...
 * 4 -pipeline-latency 8 -n 4 -name t1buv_4 -include dft/simd/t1bu.h -sign 1 */
...
static const ct_desc desc = {
    4, XSIMD_STRING("t1buv_4"), twinstr, &GENUS, {11, 6, 0, 0}, 0, 0, 0};
