...
static void dobatch(const P *ego, R *ri, R *ii, R *ro, R *io, R *buf,
                    INT batchsz) {
  X(cpy2d_pair_ci)
  (ri, ii, buf, buf + 1, ego->n, WS(ego->is, 1), WS(ego->bufstride, 1), batchsz,
   ego->ivs, 2);

  if (IABS(WS(ego->os, 1)) < IABS(ego->ovs)) {
    /* transform directly to output */
    ego->k(buf, buf + 1, ro, io, ego->bufstride, ego->os, batchsz, 2, ego->ovs);
  } else {
    /* transform to buffer and copy back */
    ego->k(buf, buf + 1, buf, buf + 1, ego->bufstride, ego->bufstride, batchsz,
           2, 2);
    X(cpy2d_pair_co)
    (buf, buf + 1, ro, io, ego->n, WS(ego->bufstride, 1), WS(ego->os, 1),
     batchsz, 2, ego->ovs);
  }
}
...
static void apply_extra_iter(const plan *ego_, R *ri, R *ii, R *ro, R *io) {
  ...
  ego->k(ri + (vl - 1) * ego->ivs, ii + (vl - 1) * ego->ivs,
         ro + (vl - 1) * ego->ovs, io + (vl - 1) * ego->ovs, ego->is, ego->os,
         1, 0, 0);
}
...
static void print(const plan *ego_, printer *p) {
  ...
  if (ego->slv->bufferedp)
    p->print(p, "(dft-directbuf/%D-%D%v \"%s\")", compute_batchsize(d->sz),
             d->sz, ego->vl, d->nam);
  else
...
static int applicable_buf(const solver *ego_, const problem *p_,
  ...
  return (1 && p->sz->rnk == 1 && p->vecsz->rnk == 1 &&
          ...
          /* UGLY if IS <= IVS */
          && !(NO_UGLYP(plnr) && X(iabs)(p->sz->dims[0].is) <= X(iabs)(ivs))

          && (batchsz = compute_batchsize(d->sz), 1) &&
          (d->genus->okp(d, 0, ((const R *)0) + 1, p->ro, p->io, 2 * batchsz,
                         p->sz->dims[0].os, batchsz, 2, ovs, plnr)) &&
          (d->genus->okp(d, 0, ((const R *)0) + 1, p->ro, p->io, 2 * batchsz,
                         p->sz->dims[0].os, vl % batchsz, 2, ovs, plnr))
...
static int applicable(const solver *ego_, const problem *p_,
  ...
  return (1 && p->sz->rnk == 1 && p->vecsz->rnk <= 1 &&
          ...
          && ((*extra_iterp = 0,
               (d->genus->okp(d, p->ri, p->ii, p->ro, p->io, p->sz->dims[0].is,
                              p->sz->dims[0].os, vl, ivs, ovs, plnr))) ||
              (*extra_iterp = 1,
               ((d->genus->okp(d, p->ri, p->ii, p->ro, p->io, p->sz->dims[0].is,
                               p->sz->dims[0].os, vl - 1, ivs, ovs, plnr)) &&
                (d->genus->okp(d, p->ri, p->ii, p->ro, p->io, p->sz->dims[0].is,
                               p->sz->dims[0].os, 2, 0, 0, plnr)))))
...
static plan *mkplan(const solver *ego_, const problem *p_, planner *plnr) {
  ...
  static const plan_adt padt = {X(dft_solve), X(null_awake), print, destroy};
