...
static void print(const plan *ego_, printer *p) {
  const P *ego = (const P *)ego_;
  p->print(p, "(dft-buffered-%D%v/%D-%D%(%p%)%(%p%)%(%p%))", ego->n, ego->nbuf,
...
static plan *mkplan(const solver *ego_, const problem *p_, planner *plnr) {
  ...
  cld = X(mkplan_f_d)(
      plnr,
      X(mkproblem_dft_d)(X(mktensor_1d)(n, p->sz->dims[0].is, 2),
                         X(mktensor_1d)(nbuf, ivs, bufdist * 2),
                         TAINT(p->ri, ivs * nbuf), TAINT(p->ii, ivs * nbuf),
  ...
  cldcpy = X(mkplan_d)(
      plnr, X(mkproblem_dft_d)(
                X(mktensor_0d)(),
                X(mktensor_2d)(nbuf, bufdist * 2, ovs, n, 2, p->sz->dims[0].os),
                bufs + roffset, bufs + ioffset, TAINT(p->ro, ovs * nbuf),
                TAINT(p->io, ovs * nbuf)));
  if (!cldcpy)
    goto nada;
