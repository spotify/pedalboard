...
static int applicable(INT irs, INT ors, INT ivs, INT ovs, const planner *plnr) {
  return (1 && irs == ors && ivs == ovs && !NO_SLOWP(plnr));
}
...
static void print(const plan *ego_, printer *p) {
  ...
  p->print(p, "(dftw-generic-%s-%D-%D%v%(%p%))",
           ego->dec == DECDIT ? "dit" : "dif", ego->r, ego->m, ego->v,
           ego->cld);
}
...
static plan *mkcldw(const ct_solver *ego_, INT r, INT irs, INT ors, INT m,
  ...
  cld = X(mkplan_d)(
      plnr, X(mkproblem_dft_d)(X(mktensor_1d)(r, irs, irs),
