...
static void print(const plan *ego_, printer *p) {
  const P *ego = (const P *)ego_;
  p->print(p, "(indirect-transpose%v%(%p%)%(%p%)%(%p%))", ego->vl,
  ...
  *pdim0 = *pdim1 = -1;
  for (dim0 = 0; dim0 < vs->rnk; ++dim0)
    for (dim1 = 0; dim1 < s->rnk; ++dim1)
      if (vs->dims[dim0].n * X(iabs)(vs->dims[dim0].is) <=
              X(iabs)(s->dims[dim1].is) &&
...
static int applicable0(const solver *ego_, const problem *p_,
                       const planner *plnr, int *pdim0, int *pdim1) {
  const problem_dft *p = (const problem_dft *)p_;
  UNUSED(ego_);
  UNUSED(plnr);

  return (1 && FINITE_RNK(p->vecsz->rnk) &&
          FINITE_RNK(p->sz->rnk)
...
static int applicable(const solver *ego_, const problem *p_,
                      const planner *plnr, int *pdim0, int *pdim1) {
  if (!applicable0(ego_, p_, plnr, pdim0, pdim1))
    return 0;
  {
    const problem_dft *p = (const problem_dft *)p_;
    ...
    if (NO_UGLYP(plnr) && p->vecsz->dims[*pdim0].is != u &&
        !(p->vecsz->rnk == 2 && p->vecsz->dims[1 - *pdim0].is == u &&
          p->vecsz->dims[*pdim0].is == u * p->vecsz->dims[1 - *pdim0].n))
      return 0;

    if (NO_INDIRECT_OP_P(plnr) && p->ri != p->ro)
      return 0;
  }
  return 1;
}
...
static plan *mkplan(const solver *ego_, const problem *p_, planner *plnr) {
  ...
  static const plan_adt padt = {X(dft_solve), awake, print, destroy};
  ...
  cldtrans = X(mkplan_d)(plnr, X(mkproblem_dft_d)(X(mktensor_0d)(),
                                                  X(tensor_append)(tv, ts), rit,
                                                  iit, rot, iot));
  ...
  tv->dims[pdim0].n = p->sz->dims[pdim1].n;
  cld = X(mkplan_d)(plnr, X(mkproblem_dft_d)(ts, tv, rot, iot, rot, iot));
  if (!cld)
    goto nada;
  ...
  cldrest =
      X(mkplan_d)(plnr, X(mkproblem_dft_d)(X(tensor_copy)(p->sz), tv,
                                           p->ri + ivs * vl, p->ii + ivs * vl,
                                           p->ro + ovs * vl, p->io + ovs * vl));
  if (!cldrest)
    goto nada;
