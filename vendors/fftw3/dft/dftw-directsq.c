...
static void apply(const plan *ego_, R *rio, R *iio) {
  ...
  ego->k(rio + mb * ms, iio + mb * ms, ego->td->W, ego->rs, ego->vs, mb,
         ego->me, ms);
}
...
static void awake(plan *ego_, enum wakefulness wakefulness) {
  ...
  X(twiddle_awake)
  (wakefulness, &ego->td, ego->slv->desc->tw, ego->r * ego->m, ego->r, ego->m);
}
...
static void print(const plan *ego_, printer *p) {
  ...
  p->print(p, "(dftw-directsq-%D/%D%v \"%s\")", ego->r,
...
static void regone(planner *plnr, kdftwsq codelet, const ct_desc *desc,
                   int dec) {
