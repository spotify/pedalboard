...
INT X(tensor_min_istride)(const tensor *sz) tensor_min_xstride(sz, is) INT
    X(tensor_min_ostride)(const tensor *sz) tensor_min_xstride(sz, os)
...
int X(tensor_strides_decrease)(const tensor *sz, const tensor *vecsz,
  ...
  return (tensor_strides_decrease(sz, k) ||
          (X(tensor_inplace_strides)(sz) && tensor_strides_decrease(vecsz, k)));
}
