...
#define MAXNAM                                                                 \
  64 /* maximum length of registrar's name.                                    \
...
static unsigned slookup(planner *ego, char *nam, int id) {
  ...
  FORALL_SOLVERS(ego, s, sp, {
    UNUSED(s);
    if (sp->reg_id == id && sp->nam_hash == h && !strcmp(sp->reg_nam, nam))
...
static solution *hlookup(planner *ego, const md5sig s, const flags_t *flagsp) {
  solution *sol = htab_lookup(&ego->htab_blessed, s, flagsp);
  if (!sol)
    sol = htab_lookup(&ego->htab_unblessed, s, flagsp);
  return sol;
}
...
static unsigned minsz(unsigned nelem) { return 1U + nelem + nelem / 8U; }

static unsigned nextsz(unsigned nelem) { return minsz(minsz(nelem)); }
...
static void hinsert(planner *ego, const md5sig s, const flags_t *flagsp,
                    unsigned slvndx) {
  htab_insert(BLISS(*flagsp) ? &ego->htab_blessed : &ego->htab_unblessed, s,
              flagsp, slvndx);
}
...
double X(iestimate_cost)(const planner *ego, const plan *pln,
                         const problem *p) {
...
static plan *search(planner *ego, const problem *p, unsigned *slvndx,
  ...
  static const unsigned relax_tab[] = {0, /* relax nothing */
                                       NO_VRECURSE, NO_FIXED_RADIX_LARGE_N,
                                       NO_SLOW, NO_UGLY};
...
skip_search:
  if (ego->wisdom_state == WISDOM_NORMAL || ego->wisdom_state == WISDOM_ONLY) {
    if (pln) {
  ...
  for (h = 0; h < ht->hashsiz; ++h) {
    ...
    if (LIVEP(l)) {
      ...
      p->print(p, "  (%s %d #x%x #x%x #x%x #x%M #x%M #x%M #x%M)\n", reg_nam,
               reg_id, l->flags.l, l->flags.u, l->flags.timelimit_impatience,
...
static int imprt(planner *ego, scanner *sc) {
  ...
  if (!sc->scan(sc, "(" WISDOM_PREAMBLE " #x%M #x%M #x%M #x%M\n", sig + 0,
                sig + 1, sig + 2, sig + 3))
    return 0; /* don't need to restore hashtable */

  signature_of_configuration(&m, ego);
  if (m.s[0] != sig[0] || m.s[1] != sig[1] || m.s[2] != sig[2] ||
      m.s[3] != sig[3]) {
  ...
  while (1) {
    ...
    if (!sc->scan(sc, "(%*s %d #x%x #x%x #x%x #x%M #x%M #x%M #x%M)", MAXNAM,
                  buf, &reg_id, &l, &u, &timelimit_impatience, sig + 0, sig + 1,
                  sig + 2, sig + 3))
      goto bad;
...
planner *X(mkplanner)(void) {
  ...
  static const planner_adt padt = {register_solver, mkplan, forget, exprt,
                                   imprt};
