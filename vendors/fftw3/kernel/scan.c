...
static void myungetc(scanner *sc, int c) { sc->ungotc = c; }
...
static void mygets(scanner *sc, char *s, int maxlen) {
  ...
  A(maxlen > 0);
  while ((ch = GETCHR(sc)) != EOF && !isspace(ch) && ch != ')' && ch != '(' &&
         s < s0 + maxlen)
    *s++ = (char)(ch & 0xFF);
  *s = 0;
  UNGETCHR(sc, ch);
  ...
  while ((c = *s++)) {
    ...
    getformat:
      ...
      case 'M': {
        ...
        *x = (md5uint)(0xFFFFFFFF & getlong(sc, 16, &ch));
        if (!ch)
          return 0;
        break;
      }
      case '*': {
        if ((fmt_len = va_arg(ap, int)) <= 0)
          return 0;
        goto getformat;
      }
      default:
...
void X(scanner_destroy)(scanner *sc) { X(ifree)(sc); }
