...
#if defined(WITH_OUR_MALLOC) && (MIN_ALIGNMENT >= 8) &&                        \
    IS_POWER_OF_TWO(MIN_ALIGNMENT)
...
static void *our_malloc(size_t n) {
  ...
  if (!(p0 = malloc(n + MIN_ALIGNMENT)))
    return (void *)0;
  p = (void *)(((uintptr_t)p0 + MIN_ALIGNMENT) &
...
#error                                                                         \
    "Don't know how to malloc() aligned memory ... try configuring --with-our-malloc"
#endif
...
void X(kernel_free)(void *p) { real_free(p); }
