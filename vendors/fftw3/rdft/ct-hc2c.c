...
static void print(const plan *ego_, printer *p) {
  ...
  p->print(p, "(rdft2-ct-%s/%D%(%p%)%(%p%))",
           (ego->super.apply == apply_dit || ego->super.apply == apply_dit_dft)
               ? "dit"
               : "dif",
           ego->r, ego->cldw, ego->cld);
}

static int applicable0(const hc2c_solver *ego, const problem *p_,
                       planner *plnr) {
  ...
  return (1 && p->sz->rnk == 1 && p->vecsz->rnk <= 1
          ...
          && (/* either the problem is R2HC, which is solved by DIT */
              ...
              (p->kind == HC2R && (p->r0 == p->cr || !NO_DESTROY_INPUTP(plnr))))
...
static int hc2c_applicable(const hc2c_solver *ego, const problem *p_,
  ...
  return (0 || p->vecsz->rnk == 0 || !NO_VRECURSEP(plnr));
}
...
static plan *mkplan(const solver *ego_, const problem *p_, planner *plnr) {
  ...
  static const plan_adt padt = {X(rdft2_solve), awake, print, destroy};
  ...
  switch (p->kind) {
  case R2HC:
    cldw = ego->mkcldw(ego, R2HC, r, m * d[0].os, m, d[0].os, v, ovs, p->cr,
                       p->ci, plnr);
    if (!cldw)
      goto nada;
    ...
    case HC2C_VIA_RDFT:
      cld = X(mkplan_d)(
          plnr, X(mkproblem_rdft_1_d)(
                    ...
                    X(mktensor_3d)(2, p->r1 - p->r0, p->ci - p->cr, r / 2,
                                   d[0].is, m * d[0].os, v, ivs, ovs),
                    p->r0, p->cr, R2HC));
      if (!cld)
        goto nada;
    ...
    case HC2C_VIA_DFT:
      cld = X(mkplan_d)(
          plnr, X(mkproblem_dft_d)(
                    X(mktensor_1d)(m, (r / 2) * d[0].is, d[0].os),
                    X(mktensor_2d)(r / 2, d[0].is, m * d[0].os, v, ivs, ovs),
                    p->r0, p->r1, p->cr, p->ci));
      if (!cld)
        goto nada;
  ...
  case HC2R:
    cldw = ego->mkcldw(ego, HC2R, r, m * d[0].is, m, d[0].is, v, ivs, p->cr,
                       p->ci, plnr);
    if (!cldw)
      goto nada;
    ...
    case HC2C_VIA_RDFT:
      cld = X(mkplan_d)(
          plnr, X(mkproblem_rdft_1_d)(
                    ...
                    X(mktensor_3d)(2, p->ci - p->cr, p->r1 - p->r0, r / 2,
                                   m * d[0].is, d[0].os, v, ivs, ovs),
                    p->cr, p->r0, HC2R));
      if (!cld)
        goto nada;
    ...
    case HC2C_VIA_DFT:
      cld = X(mkplan_d)(
          plnr, X(mkproblem_dft_d)(
                    X(mktensor_1d)(m, d[0].is, (r / 2) * d[0].os),
                    X(mktensor_2d)(r / 2, m * d[0].is, d[0].os, v, ivs, ovs),
                    p->ci, p->cr, p->r1, p->r0));
      if (!cld)
        goto nada;
...
hc2c_solver *X(mksolver_hc2c)(size_t size, INT r, hc2c_kind hc2ckind,
