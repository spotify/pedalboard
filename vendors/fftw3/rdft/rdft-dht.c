...
static void print(const plan *ego_, printer *p) {
  const P *ego = (const P *)ego_;
  p->print(p, "(%s-dht-%D%(%p%))",
           ego->super.apply == apply_r2hc ? "r2hc" : "hc2r", ego->n, ego->cld);
}
...
static plan *mkplan(const solver *ego_, const problem *p_, planner *plnr) {
  ...
  pln = MKPLAN_RDFT(
      ...
      p->kind[0] == R2HC
          ? apply_r2hc
          : (NO_DESTROY_INPUTP(plnr) ? apply_hc2r_save : apply_hc2r));
  pln->n = p->sz->dims[0].n;
...
void X(rdft_dht_register)(planner *p) { REGISTER_SOLVER(p, mksolver()); }
