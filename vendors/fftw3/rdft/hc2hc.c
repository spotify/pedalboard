...
static int applicable0(const hc2hc_solver *ego, const problem *p_,
                       planner *plnr) {
  ...
  return (1 && p->sz->rnk == 1 && p->vecsz->rnk <= 1
          ...
          (/* either the problem is R2HC, which is solved by DIT */
           ...
           (p->kind[0] == HC2R && (p->I == p->O || !NO_DESTROY_INPUTP(plnr))))

          && ((r = X(choose_radix)(ego->r, p->sz->dims[0].n)) > 0) &&
          p->sz->dims[0].n > r);
}

int X(hc2hc_applicable)(const hc2hc_solver *ego, const problem *p_,
                        planner *plnr) {
...
static plan *mkplan(const solver *ego_, const problem *p_, planner *plnr) {
  ...
  switch (p->kind[0]) {
  case R2HC:
    cldw = ego->mkcldw(ego, R2HC, r, m, d[0].os, v, ovs, 0, (m + 2) / 2, p->O,
                       plnr);
    if (!cldw)
      goto nada;
  ...
  case HC2R:
    cldw = ego->mkcldw(ego, HC2R, r, m, d[0].is, v, ivs, 0, (m + 2) / 2, p->I,
                       plnr);
    if (!cldw)
      goto nada;
    ...
    cld = X(mkplan_d)(plnr,
                      X(mkproblem_rdft_d)(
                          ...
                          X(mktensor_2d)(r, m * d[0].is, d[0].os, v, ivs, ovs),
                          p->I, p->O, p->kind));
    if (!cld)
      goto nada;
