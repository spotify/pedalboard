...
static int fill_iodim(P *pln, const problem_rdft *p) {
  ...
  for (i = 0; i < vecsz->rnk; ++i) {
    ...
    if (pln->vl == 1 && vecsz->dims[i].is == 1 && vecsz->dims[i].os == 1)
...
static void copy(const iodim *d, int rnk, INT vl, R *I, R *O,
                 cpy2d_func cpy2d) {
  A(rnk >= 2);
  if (rnk == 2)
...
static int transposep(const P *pln) {
  ...
  return (pln->d[i].n == pln->d[i + 1].n && pln->d[i].is == pln->d[i + 1].os &&
          pln->d[i].os == pln->d[i + 1].is);
}

/* generic higher-rank transpose routine, calls transpose2d() to do
 * the real work */
static void transpose(const iodim *d, int rnk, INT vl, R *I,
...
static void apply_iter(const plan *ego_, R *I, R *O) {
  ...
  case 0:
    ...
    break;
  case 1:
    X(cpy1d)(I, O, ego->d[0].n, ego->d[0].is, ego->d[0].os, ego->vl);
    break;
  default:
...
static int applicable_ip_sq(const P *pln, const problem_rdft *p) {
  return (1 && p->I == p->O && pln->rnk >= 2 && transposep(pln));
}
...
static int applicable(const S *ego, const problem *p_) {
  ...
  P pln;
  return (1 && p->sz->rnk == 0 && FINITE_RNK(p->vecsz->rnk) &&
          fill_iodim(&pln, p) && ego->applicable(&pln, p));
}
...
static plan *mkplan(const solver *ego_, const problem *p_, planner *plnr) {
  ...
  static const plan_adt padt = {X(rdft_solve), X(null_awake), print,
                                X(plan_null_destroy)};
...
void X(rdft_rank0_register)(planner *p) {
  ...
  static struct {
    ...
    const char *nam;
      ...
      {apply_memcpy_loop, applicable_memcpy_loop, "rdft-rank0-memcpy-loop"},
      ...
      {apply_ip_sq_tiled, applicable_ip_sq_tiled, "rdft-rank0-ip-sq-tiled"},
      {apply_ip_sq_tiledbuf, applicable_ip_sq_tiledbuf,
