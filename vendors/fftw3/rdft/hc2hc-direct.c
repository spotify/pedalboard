...
static void apply(const plan *ego_, R *IO) {
  ...
  for (i = 0; i < v; ++i, IO += vs) {
    ...
    ego->k(IO + ms * mb, IO + (m - mb) * ms, ego->td->W, ego->rs, mb, me, ms);
...
static void dobatch(const P *ego, R *IOp, R *IOm, INT mb, INT me, R *bufp) {
  INT b = WS(ego->brs, 1);
  INT rs = WS(ego->rs, 1);
  INT r = ego->r;
  INT ms = ego->ms;
...
static void print(const plan *ego_, printer *p) {
  ...
  if (slv->bufferedp)
    p->print(p, "(hc2hc-directbuf/%D-%D/%D%v \"%s\"%(%p%)%(%p%))", batchsz,
             ego->r, X(twiddle_length)(ego->r, e->tw), ego->v, e->nam,
             ego->cld0, ego->cldm);
  else
    p->print(p, "(hc2hc-direct-%D/%D%v \"%s\"%(%p%)%(%p%))", ego->r,
             X(twiddle_length)(ego->r, e->tw), ego->v, e->nam, ego->cld0,
             ego->cldm);
}
...
static int applicable0(const S *ego, rdft_kind kind, INT r) {
  ...
  return (1 && r == e->radix && kind == e->genus->kind);
}
...
static plan *mkcldw(const hc2hc_solver *ego_, rdft_kind kind, INT r, INT m,
                    INT ms, INT v, INT vs, INT mstart, INT mcount, R *IO,
                    planner *plnr) {
  ...
  cld0 = X(mkplan_d)(
      plnr, X(mkproblem_rdft_1_d)(
                (CLD0P(mstart) ? X(mktensor_1d)(r, rs, rs) : X(mktensor_0d)()),
                X(mktensor_0d)(), TAINT(IO, vs), TAINT(IO, vs), kind));
  if (!cld0)
    goto nada;

  cldm = X(mkplan_d)(
      plnr, X(mkproblem_rdft_1_d)(
                (CLDMP(m, mstart, mcount) ? X(mktensor_1d)(r, rs, rs)
                                          : X(mktensor_0d)()),
                X(mktensor_0d)(), TAINT(IO + imid, vs), TAINT(IO + imid, vs),
  ...
  X(ops_madd2)
  (v * ((pln->me - pln->mb) / e->genus->vl), &e->ops, &pln->super.super.ops);
