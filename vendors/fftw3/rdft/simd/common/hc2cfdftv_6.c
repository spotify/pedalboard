 ...
 * -no-generate-bytw -n 6 -dit -name hc2cfdftv_6 -include rdft/simd/hc2cfv.h */
...
static void hc2cfdftv_6(R *Rp, R *Ip, R *Rm, R *Im, const R *W, stride rs,
                        INT mb, INT me, INT ms) {
  DVK(KP866025403, +0.866025403784438646763723170752936183471402627);
  DVK(KP500000000, +0.500000000000000000000000000000000000000000000);
  {
    INT m;
    for (m = mb, W = W + ((mb - 1) * ((TWVL / VL) * 10)); m < me; m = m + VL,
        Rp = Rp + (VL * ms), Ip = Ip + (VL * ms), Rm = Rm - (VL * ms),
        Im = Im - (VL * ms), W = W + (TWVL * 10),
        MAKE_VOLATILE_STRIDE(24, rs)) {
...
static const tw_instr twinstr[] = {VTW(1, 1), VTW(1, 2), VTW(1, 3),
                                   VTW(1, 4), VTW(1, 5), {TW_NEXT, VL, 0}};

static const hc2c_desc desc = {
    6, XSIMD_STRING("hc2cfdftv_6"), twinstr, &GENUS, {17, 18, 12, 0}};
 ...
 * 4 -pipeline-latency 8 -trivial-stores -variables 32 -no-generate-bytw -n 6
 * -dit -name hc2cfdftv_6 -include rdft/simd/hc2cfv.h */
...
static const hc2c_desc desc = {
    6, XSIMD_STRING("hc2cfdftv_6"), twinstr, &GENUS, {27, 18, 2, 0}};
