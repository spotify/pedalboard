 ...
 * -no-generate-bytw -n 2 -dit -name hc2cfdftv_2 -include rdft/simd/hc2cfv.h */
...
static void hc2cfdftv_2(R *Rp, R *Ip, R *Rm, R *Im, const R *W, stride rs,
                        INT mb, INT me, INT ms) {
  DVK(KP500000000, +0.500000000000000000000000000000000000000000000);
  {
    INT m;
    for (m = mb, W = W + ((mb - 1) * ((TWVL / VL) * 2)); m < me; m = m + VL,
        Rp = Rp + (VL * ms), Ip = Ip + (VL * ms), Rm = Rm - (VL * ms),
        Im = Im - (VL * ms), W = W + (TWVL * 2), MAKE_VOLATILE_STRIDE(8, rs)) {
...
static const tw_instr twinstr[] = {VTW(1, 1), {TW_NEXT, VL, 0}};

static const hc2c_desc desc = {
    2, XSIMD_STRING("hc2cfdftv_2"), twinstr, &GENUS, {3, 4, 2, 0}};
 ...
 * 4 -pipeline-latency 8 -trivial-stores -variables 32 -no-generate-bytw -n 2
 * -dit -name hc2cfdftv_2 -include rdft/simd/hc2cfv.h */
...
static const hc2c_desc desc = {
    2, XSIMD_STRING("hc2cfdftv_2"), twinstr, &GENUS, {5, 4, 0, 0}};
