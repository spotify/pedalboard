...
static void mktwiddle(P *ego, enum wakefulness wakefulness) {
  ...
  X(twiddle_awake)(wakefulness, &ego->td, tw, ego->r * ego->m, ego->m, ego->r);
}
...
static int applicable(rdft_kind kind, INT r, INT m, const planner *plnr) {
  return (1 && (kind == R2HC || kind == HC2R) && (m % 2) && (r % 2) &&
          !NO_SLOWP(plnr));
}
...
static void print(const plan *ego_, printer *p) {
  ...
  p->print(p, "(hc2hc-generic-%s-%D-%D%v%(%p%)%(%p%))",
           ego->super.apply == apply_dit ? "dit" : "dif", ego->r, ego->m,
           ego->vl, ego->cld0, ego->cld);
}

static plan *mkcldw(const hc2hc_solver *ego_, rdft_kind kind, INT r, INT m,
                    INT s, INT vl, INT vs, INT mstart, INT mcount, R *IO,
                    planner *plnr) {
  P *pln;
  plan *cld0 = 0, *cld = 0;
  ...
  cld0 = X(mkplan_d)(
      plnr, X(mkproblem_rdft_1_d)(mstart == 0 ? X(mktensor_1d)(r, m * s, m * s)
                                              : X(mktensor_0d)(),
                                  X(mktensor_1d)(vl, vs, vs), IO, IO, kind));
  if (!cld0)
    goto nada;
  ...
  cld = X(mkplan_d)(
      plnr, X(mkproblem_rdft_1_d)(X(mktensor_1d)(r, m * s, m * s),
                                  X(mktensor_3d)(2, mstride * s, mstride * s,
                                                 mcount1, s, s, vl, vs, vs),
...
void X(hc2hc_generic_register)(planner *p) { regsolver(p, 0); }
