...
static void print(const plan *ego_, printer *p) {
  ...
  p->print(p, "(rdft-vrank>=1-x%D/%d%(%p%))", ego->vl, s->vecloop_dim,
           ego->cld);
}

static int pickdim(const S *ego, const tensor *vecsz, int oop, int *dp) {
  return X(pickdim)(ego->vecloop_dim, ego->buddies, ego->nbuddies, vecsz, oop,
                    dp);
}
...
static int applicable0(const solver *ego_, const problem *p_, int *dp) {
  ...
  return (1 && FINITE_RNK(p->vecsz->rnk) && p->vecsz->rnk > 0
...
static int applicable(const solver *ego_, const problem *p_,
  ...
  if (NO_UGLYP(plnr)) {
       ...
       this vector with the transform-dimension vectors. */
    {
      ...
      if (1 && p->sz->rnk > 1 &&
          X(imin)(X(iabs)(d->is), X(iabs)(d->os)) < X(tensor_max_index)(p->sz))
        return 0;
    }
    ...
    if (p->vecsz->rnk == 1 && p->sz->rnk == 1 && REODFT_KINDP(p->kind[0]))
      return 0;
  }
...
static plan *mkplan(const solver *ego_, const problem *p_, planner *plnr) {
  ...
  cld = X(mkplan_d)(plnr, X(mkproblem_rdft_d)(
                              X(tensor_copy)(p->sz),
                              X(tensor_copy_except)(p->vecsz, vdim),
                              TAINT(p->I, d->is), TAINT(p->O, d->os), p->kind));
  if (!cld)
    return (plan *)0;
