...
static void print(const plan *ego_, printer *p) {
  ...
  p->print(p, "(rdft2-rank>=2/%d%(%p%)%(%p%))", s->spltrnk, ego->cldr,
           ego->cldc);
}
...
static int applicable0(const solver *ego_, const problem *p_, int *rp,
  ...
  return (1 && FINITE_RNK(p->sz->rnk) &&
          FINITE_RNK(p->vecsz->rnk)

          /* FIXME: multidimensional R2HCII ? */
          && (p->kind == R2HC || p->kind == HC2R)

          && p->sz->rnk >= 2 && picksplit(ego, p->sz, rp) &&
          (0

           /* can work out-of-place, but HC2R destroys input */
           || (p->r0 != p->cr && (p->kind == R2HC || !NO_DESTROY_INPUTP(plnr)))
...
static int applicable(const solver *ego_, const problem *p_,
  ...
  if (!applicable0(ego_, p_, rp, plnr))
    return 0;
  ...
  if (NO_UGLYP(plnr)) {
    ...
    /* Heuristic: if the vector stride is greater than the transform
       ...
       vrank-geq1 plan). */
    if (p->vecsz->rnk > 0 && X(tensor_min_stride)(p->vecsz) >
...
static plan *mkplan(const solver *ego_, const problem *p_, planner *plnr) {
  ...
  cldr = X(mkplan_d)(plnr,
                     X(mkproblem_rdft2_d)(X(tensor_copy)(sz2),
                                          ...
                                          p->r0, p->r1, p->cr, p->ci, p->kind));
  if (!cldr)
    goto nada;

  if (p->kind == R2HC)
    cldp = X(mkproblem_dft_d)(X(tensor_copy_inplace)(sz1, k),
                              X(tensor_append)(vecszi, sz2i), p->cr, p->ci,
                              p->cr, p->ci);
  else /* HC2R must swap re/im parts to get IDFT */
    cldp = X(mkproblem_dft_d)(X(tensor_copy_inplace)(sz1, k),
                              X(tensor_append)(vecszi, sz2i), p->ci, p->cr,
                              p->ci, p->cr);
