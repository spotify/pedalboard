...
static int applicable(const solver *ego_, const problem *p_) {
  ...
  return (0
          ...
          /* case 2 : rank-0 in-place rdft, except that
             ...
             part to 0 */
          || (1 && p->kind != R2HC && p->sz->rnk == 0 &&
              FINITE_RNK(p->vecsz->rnk) && (p->r0 == p->cr) &&
...
static plan *mkplan(const solver *ego, const problem *p, planner *plnr) {
  static const plan_adt padt = {X(rdft2_solve), X(null_awake), print,
                                X(plan_null_destroy)};
  plan_rdft2 *pln;
...
void X(rdft2_nop_register)(planner *p) { REGISTER_SOLVER(p, mksolver()); }
