...
static void apply(const plan *ego_, R *r0, R *r1, R *cr, R *ci) {
  ...
  for (i = 0; i < vl; ++i) {
    cldapply(ego->cld, r0 + i * rvs, r1 + i * rvs, cr + i * cvs, ci + i * cvs);
  }
}
...
static void print(const plan *ego_, printer *p) {
  ...
  p->print(p, "(rdft2-vrank>=1-x%D/%d%(%p%))", ego->vl, s->vecloop_dim,
           ego->cld);
}
...
static int applicable(const solver *ego_, const problem *p_,
  ...
  if (NO_UGLYP(plnr)) {
    ...
    if (p->sz->rnk > 1 && X(imin)(X(iabs)(d->is), X(iabs)(d->os)) <
...
static plan *mkplan(const solver *ego_, const problem *p_, planner *plnr) {
  ...
  cld = X(mkplan_d)(plnr, X(mkproblem_rdft2_d)(
                              X(tensor_copy)(p->sz),
                              X(tensor_copy_except)(p->vecsz, vdim),
                              TAINT(p->r0, rvs), TAINT(p->r1, rvs),
                              TAINT(p->cr, cvs), TAINT(p->ci, cvs), p->kind));
  if (!cld)
    return (plan *)0;
