...
static void apply(const plan *ego_, R *cr, R *ci) {
  ...
  for (i = 0; i < v; ++i, cr += vs, ci += vs) {
    ...
    ego->k(cr + ms, ci + ms, cr + (m - 1) * ms, ci + (m - 1) * ms, ego->td->W,
           ego->rs, 1, (m + 1) / 2, ms);
...
static void dobatch(const P *ego, R *Rp, R *Ip, R *Rm, R *Im, INT mb, INT me,
                    INT extra_iter, R *bufp) {
  INT b = WS(ego->brs, 1);
  INT rs = WS(ego->rs, 1);
  INT ms = ego->ms;
  R *bufm = bufp + b - 2;
  ...
  X(cpy2d_pair_ci)
  (Rp + mb * ms, Ip + mb * ms, bufp, bufp + 1, ego->r / 2, rs, b, n, ms, 2);
  X(cpy2d_pair_ci)
  (Rm - mb * ms, Im - mb * ms, bufm, bufm + 1, ego->r / 2, rs, b, n, -ms, -2);
  ...
  ego->k(bufp, bufp + 1, bufm, bufm + 1, ego->td->W, ego->brs, mb,
         me + extra_iter, 2);
  X(cpy2d_pair_co)
  (bufp, bufp + 1, Rp + mb * ms, Ip + mb * ms, ego->r / 2, b, rs, n, 2, ms);
  X(cpy2d_pair_co)
  (bufm, bufm + 1, Rm - mb * ms, Im - mb * ms, ego->r / 2, b, rs, n, -2, -ms);
}
...
static void apply_buf(const plan *ego_, R *cr, R *ci) {
  ...
  for (i = 0; i < v; ++i, cr += ego->vs, ci += ego->vs) {
    ...
    cldm->apply((plan *)cldm, Rp + me * ms, Ip + me * ms, Rp + me * ms,
                Ip + me * ms);
  }
...
static void awake(plan *ego_, enum wakefulness wakefulness) {
  ...
  X(twiddle_awake)
  (wakefulness, &ego->td, ego->slv->desc->tw, ego->r * ego->m, ego->r,
...
static void print(const plan *ego_, printer *p) {
  ...
  if (slv->bufferedp)
    p->print(p, "(hc2c-directbuf/%D-%D/%D/%D%v \"%s\"%(%p%)%(%p%))",
             ...
             X(twiddle_length)(ego->r, e->tw), ego->extra_iter, ego->v, e->nam,
             ego->cld0, ego->cldm);
  else
    p->print(p, "(hc2c-direct-%D/%D/%D%v \"%s\"%(%p%)%(%p%))", ego->r,
             X(twiddle_length)(ego->r, e->tw), ego->extra_iter, ego->v, e->nam,
             ego->cld0, ego->cldm);
}

static int applicable0(const S *ego, rdft_kind kind, INT r, INT rs, INT m,
                       INT ms, INT v, INT vs, const R *cr, const R *ci,
...
static int applicable0_buf(const S *ego, rdft_kind kind, INT r, INT rs, INT m,
                           INT ms, INT v, INT vs, const R *cr, const R *ci,
                           const planner *plnr, INT *extra_iter) {
  const hc2c_desc *e = ego->desc;
  INT batchsz, brs;
  ...
  return (1 && r == e->radix &&
          ...
          && (cr = (const R *)0, ci = cr + 1, batchsz = compute_batchsize(r),
              brs = 4 * batchsz, 1)

          && e->genus->okp(cr, ci, cr + brs - 2, ci + brs - 2, brs, 1,
                           1 + batchsz, 2, plnr)

          && ((*extra_iter = 0,
               e->genus->okp(cr, ci, cr + brs - 2, ci + brs - 2, brs, 1,
                             1 + (((m - 1) / 2) % batchsz), 2, plnr)) ||
              (*extra_iter = 1,
               e->genus->okp(cr, ci, cr + brs - 2, ci + brs - 2, brs, 1,
                             1 + 1 + (((m - 1) / 2) % batchsz), 2, plnr)))

  );
}

static int applicable(const S *ego, rdft_kind kind, INT r, INT rs, INT m,
                      INT ms, INT v, INT vs, R *cr, R *ci, const planner *plnr,
                      INT *extra_iter) {
  if (ego->bufferedp) {
    ...
    if (!applicable0(ego, kind, r, rs, m, ms, v, vs, cr, ci, plnr, extra_iter))
      return 0;
  }

  if (NO_UGLYP(plnr) &&
      X(ct_uglyp)((ego->bufferedp ? (INT)512 : (INT)16), v, m * r, r))
    return 0;

  return 1;
}

static plan *mkcldw(const hc2c_solver *ego_, rdft_kind kind, INT r, INT rs,
                    INT m, INT ms, INT v, INT vs, R *cr, R *ci, planner *plnr) {
  ...
  static const plan_adt padt = {0, awake, print, destroy};

  if (!applicable(ego, kind, r, rs, m, ms, v, vs, cr, ci, plnr, &extra_iter))
    return (plan *)0;

  cld0 = X(mkplan_d)(plnr, X(mkproblem_rdft2_d)(X(mktensor_1d)(r, rs, rs),
                                                X(mktensor_0d)(), TAINT(cr, vs),
                                                TAINT(ci, vs), TAINT(cr, vs),
                                                TAINT(ci, vs), kind));
  if (!cld0)
    goto nada;

  cldm = X(mkplan_d)(
      plnr, X(mkproblem_rdft2_d)(
                ((m % 2) ? X(mktensor_0d)() : X(mktensor_1d)(r, rs, rs)),
                X(mktensor_0d)(), TAINT(cr + imid, vs), TAINT(ci + imid, vs),
  ...
  X(ops_madd2)
  (v * (((m - 1) / 2) / e->genus->vl), &e->ops, &pln->super.super.ops);
...
static void regone(planner *plnr, khc2c codelet, const hc2c_desc *desc,
                   hc2c_kind hc2ckind, int bufferedp) {
...
void X(regsolver_hc2c_direct)(planner *plnr, khc2c codelet,
                              const hc2c_desc *desc, hc2c_kind hc2ckind) {
