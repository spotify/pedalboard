  ...
  return (1 && p->sz->rnk == 0 &&
          (p->kind == HC2R ||
           (1 && p->kind == R2HC
            ...
            && ((p->r0 != p->cr) || X(rdft2_inplace_strides)(p, RNK_MINFTY)))));
}
...
static plan *mkplan(const solver *ego_, const problem *p_, planner *plnr) {
  ...
  if (p->kind == HC2R) {
    cldcpy = X(mkplan_d)(
        plnr, X(mkproblem_rdft_0_d)(X(tensor_copy)(p->vecsz), p->cr, p->r0));
    if (!cldcpy)
      return (plan *)0;
  }
...
void X(rdft2_rank0_register)(planner *p) { REGISTER_SOLVER(p, mksolver()); }
