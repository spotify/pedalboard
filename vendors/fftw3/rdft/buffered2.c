...
static void print(const plan *ego_, printer *p) {
  const P *ego = (const P *)ego_;
  p->print(p, "(rdft2-buffered-%D%v/%D-%D%(%p%)%(%p%)%(%p%))", ego->n,
           ego->nbuf, ego->vl, ego->bufdist % ego->n, ego->cld, ego->cldcpy,
           ego->cldrest);
}
...
static int applicable0(const S *ego, const problem *p_, const planner *plnr) {
  ...
  if (1 && p->vecsz->rnk <= 1 &&
    ...
    /* if this solver is redundant, in the sense that a solver
       ...
       solver */
    if (X(nbuf_redundant)(d[0].n, vl, ego->maxnbuf_ndx, maxnbufs,
                          NELEM(maxnbufs)))
      return 0;
    ...
    if (/* fits into buffer: */
        ((p->vecsz->rnk == 0) ||
         (X(nbuf)(d[0].n, p->vecsz->dims[0].n, maxnbufs[ego->maxnbuf_ndx]) ==
          p->vecsz->dims[0].n)))
      return 1;
  }
...
static plan *mkplan(const solver *ego_, const problem *p_, planner *plnr) {
  ...
  if (p->kind == R2HC) {
    ...
    cld = X(mkplan_f_d)(
        plnr,
        X(mkproblem_rdft2_d)(X(mktensor_1d)(n, p->sz->dims[0].is, 2),
    ...
    cldcpy = X(mkplan_d)(
        ...
        X(mkproblem_dft_d)(
            X(mktensor_0d)(),
            X(mktensor_2d)(nbuf, bufdist, ovs, n / 2 + 1, 2, p->sz->dims[0].os),
            bufs + roffset, bufs + ioffset, TAINT(p->cr, ovs * nbuf),
            TAINT(p->ci, ovs * nbuf)));
    if (!cldcpy)
      goto nada;
    ...
    cldrest = X(mkplan_d)(
        plnr, X(mkproblem_rdft2_d)(
                  X(tensor_copy)(p->sz), X(mktensor_1d)(vl % nbuf, ivs, ovs),
                  p->r0 + id, p->r1 + id, p->cr + od, p->ci + od, p->kind));
    if (!cldrest)
      goto nada;
    ...
    cld = X(mkplan_f_d)(
        plnr,
        X(mkproblem_rdft2_d)(X(mktensor_1d)(n, 2, p->sz->dims[0].os),
    ...
    cldcpy = X(mkplan_d)(
        ...
        X(mkproblem_dft_d)(
            X(mktensor_0d)(),
            X(mktensor_2d)(nbuf, ivs, bufdist, n / 2 + 1, p->sz->dims[0].is, 2),
            TAINT(p->cr, ivs * nbuf), TAINT(p->ci, ivs * nbuf), bufs + roffset,
            bufs + ioffset));
    if (!cldcpy)
      goto nada;
    ...
    cldrest = X(mkplan_d)(
        plnr, X(mkproblem_rdft2_d)(
                  X(tensor_copy)(p->sz), X(mktensor_1d)(vl % nbuf, ivs, ovs),
                  p->r0 + od, p->r1 + od, p->cr + id, p->ci + id, p->kind));
    if (!cldrest)
      goto nada;
