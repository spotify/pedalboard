 ...
 * -pipeline-latency 4 -n 9 -dit -name hf_9 -include rdft/scalar/hf.h */
...
static void hf_9(R *cr, R *ci, const R *W, stride rs, INT mb, INT me, INT ms) {
  ...
  {
    INT m;
    for (m = mb, W = W + ((mb - 1) * 16); m < me; m = m + 1, cr = cr + ms,
        ci = ci - ms, W = W + 16, MAKE_VOLATILE_STRIDE(18, rs)) {
      ...
      {
        E T11, T1z, T1T, T1X, T1f, T1w, T1t, T1x, T1u, T1Y, T1C, T1I, T1F, T1J,
            T1G;
        E T1U, TV, T1R;
...
static const tw_instr twinstr[] = {{TW_FULL, 1, 9}, {TW_NEXT, 1, 0}};

static const hc2hc_desc desc = {9, "hf_9", twinstr, &GENUS, {24, 16, 72, 0}};

void X(codelet_hf_9)(planner *p) { X(khc2hc_register)(p, hf_9, &desc); }
#else
