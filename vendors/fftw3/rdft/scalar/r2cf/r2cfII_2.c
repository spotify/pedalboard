 ...
 * -pipeline-latency 4 -n 2 -name r2cfII_2 -dft-II -include rdft/scalar/r2cfII.h
 */
...
static void r2cfII_2(R *R0, R *R1, R *Cr, R *Ci, stride rs, stride csr,
                     stride csi, INT v, INT ivs, INT ovs) {
  {
    INT i;
    for (i = v; i > 0; i = i - 1, R0 = R0 + ivs, R1 = R1 + ivs, Cr = Cr + ovs,
        Ci = Ci + ovs, MAKE_VOLATILE_STRIDE(8, rs),
