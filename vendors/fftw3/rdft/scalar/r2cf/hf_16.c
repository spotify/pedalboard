 ...
 * -pipeline-latency 4 -n 16 -dit -name hf_16 -include rdft/scalar/hf.h */
...
static void hf_16(R *cr, R *ci, const R *W, stride rs, INT mb, INT me, INT ms) {
  ...
  {
    INT m;
    for (m = mb, W = W + ((mb - 1) * 30); m < me; m = m + 1, cr = cr + ms,
        ci = ci - ms, W = W + 30, MAKE_VOLATILE_STRIDE(32, rs)) {
      ...
      {
        E T1O, T3B, T3H, T2E, T23, T3I, T2O, T2R, T2H, T3C, T2j, T2B, T2L, T2S,
            T2y;
        E T2C;
        {
          E T1V, T22, T2b, T2i;
...
void X(codelet_hf_16)(planner *p) { X(khc2hc_register)(p, hf_16, &desc); }
#else
...
static void hf_16(R *cr, R *ci, const R *W, stride rs, INT mb, INT me, INT ms) {
  ...
  DK(KP707106781, +0.707106781186547524400844362104849039284835938);
  {
    ...
    for (m = mb, W = W + ((mb - 1) * 30); m < me; m = m + 1, cr = cr + ms,
      ...
      E T2d, T1f, T1q, T2B, T2C, T2D, T2E, T1Z, T2k, T24, T2j, TS, T13, T2w,
          T2x;
