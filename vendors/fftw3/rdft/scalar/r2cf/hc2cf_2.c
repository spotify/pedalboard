 ...
 * -pipeline-latency 4 -n 2 -dit -name hc2cf_2 -include rdft/scalar/hc2cf.h */
...
static void hc2cf_2(R *Rp, R *Ip, R *Rm, R *Im, const R *W, stride rs, INT mb,
                    INT me, INT ms) {
  {
    INT m;
    for (m = mb, W = W + ((mb - 1) * 2); m < me; m = m + 1, Rp = Rp + ms,
        Ip = Ip + ms, Rm = Rm - ms, Im = Im - ms, W = W + 2,
        MAKE_VOLATILE_STRIDE(8, rs)) {
...
static const tw_instr twinstr[] = {{TW_FULL, 1, 2}, {TW_NEXT, 1, 0}};
