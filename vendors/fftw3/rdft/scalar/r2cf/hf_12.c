 ...
 * -pipeline-latency 4 -n 12 -dit -name hf_12 -include rdft/scalar/hf.h */
...
static void hf_12(R *cr, R *ci, const R *W, stride rs, INT mb, INT me, INT ms) {
  ...
  {
    INT m;
    for (m = mb, W = W + ((mb - 1) * 22); m < me; m = m + 1, cr = cr + ms,
        ci = ci - ms, W = W + 22, MAKE_VOLATILE_STRIDE(24, rs)) {
      ...
      {
        E T1m, T1K, T2q, T2y, T2t, T2z, T1t, T1L, T1B, T1N, T1W, T25, T22, T26,
            T1I;
        E T1O;
        {
          E T1g, T2o, T2r, T1n;
...
void X(codelet_hf_12)(planner *p) { X(khc2hc_register)(p, hf_12, &desc); }
#else
