 ...
 * -pipeline-latency 4 -n 15 -dit -name hf_15 -include rdft/scalar/hf.h */
...
static void hf_15(R *cr, R *ci, const R *W, stride rs, INT mb, INT me, INT ms) {
  ...
  {
    INT m;
    for (m = mb, W = W + ((mb - 1) * 28); m < me; m = m + 1, cr = cr + ms,
        ci = ci - ms, W = W + 28, MAKE_VOLATILE_STRIDE(30, rs)) {
      E T1, T3i, T1G, T3l, Te, T1B, T3j, T3k, T1y, T2i, T2a, T2M, T37, T2Y, Tz;
      E T2e, T1O, T2t, T39, T2U, TT, T2f, T1V, T2z, T3a, T2V, T1e, T2h, T23,
          T2G;
      E T36, T2X;
      {
        E T7, T1D, Td, T1F;
...
void X(codelet_hf_15)(planner *p) { X(khc2hc_register)(p, hf_15, &desc); }
#else
...
static void hf_15(R *cr, R *ci, const R *W, stride rs, INT mb, INT me, INT ms) {
  ...
  DK(KP866025403, +0.866025403784438646763723170752936183471402627);
  {
    ...
    for (m = mb, W = W + ((mb - 1) * 28); m < me; m = m + 1, cr = cr + ms,
        ci = ci - ms, W = W + 28, MAKE_VOLATILE_STRIDE(30, rs)) {
      E T1q, T2Q, Td, T1n, T2T, T3l, T13, T1k, T1l, T2E, T2F, T3j, T1H, T1T,
          T2k;
