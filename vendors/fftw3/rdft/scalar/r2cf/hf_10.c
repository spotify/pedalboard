 ...
 * -pipeline-latency 4 -n 10 -dit -name hf_10 -include rdft/scalar/hf.h */
...
static void hf_10(R *cr, R *ci, const R *W, stride rs, INT mb, INT me, INT ms) {
  ...
  {
    INT m;
    for (m = mb, W = W + ((mb - 1) * 18); m < me; m = m + 1, cr = cr + ms,
        ci = ci - ms, W = W + 18, MAKE_VOLATILE_STRIDE(20, rs)) {
...
void X(codelet_hf_10)(planner *p) { X(khc2hc_register)(p, hf_10, &desc); }
#else
