 ...
 * -pipeline-latency 4 -sign 1 -n 4 -dif -name hb_4 -include rdft/scalar/hb.h */
...
static void hb_4(R *cr, R *ci, const R *W, stride rs, INT mb, INT me, INT ms) {
  {
    ...
    for (m = mb, W = W + ((mb - 1) * 6); m < me; m = m + 1, cr = cr + ms,
        ci = ci - ms, W = W + 6, MAKE_VOLATILE_STRIDE(8, rs)) {
...
void X(codelet_hb_4)(planner *p) { X(khc2hc_register)(p, hb_4, &desc); }
#else
