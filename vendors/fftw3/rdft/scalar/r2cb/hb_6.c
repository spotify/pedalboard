 ...
 * -pipeline-latency 4 -sign 1 -n 6 -dif -name hb_6 -include rdft/scalar/hb.h */
...
static void hb_6(R *cr, R *ci, const R *W, stride rs, INT mb, INT me, INT ms) {
  ...
  {
    INT m;
    for (m = mb, W = W + ((mb - 1) * 10); m < me; m = m + 1, cr = cr + ms,
        ci = ci - ms, W = W + 10, MAKE_VOLATILE_STRIDE(12, rs)) {
...
void X(codelet_hb_6)(planner *p) { X(khc2hc_register)(p, hb_6, &desc); }
#else
