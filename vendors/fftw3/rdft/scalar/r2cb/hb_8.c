 ...
 * -pipeline-latency 4 -sign 1 -n 8 -dif -name hb_8 -include rdft/scalar/hb.h */
...
static void hb_8(R *cr, R *ci, const R *W, stride rs, INT mb, INT me, INT ms) {
  ...
  {
    INT m;
    for (m = mb, W = W + ((mb - 1) * 14); m < me; m = m + 1, cr = cr + ms,
        ci = ci - ms, W = W + 14, MAKE_VOLATILE_STRIDE(16, rs)) {
...
void X(codelet_hb_8)(planner *p) { X(khc2hc_register)(p, hb_8, &desc); }
#else
