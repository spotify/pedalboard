 ...
 * -pipeline-latency 4 -sign 1 -n 14 -name r2cb_14 -include rdft/scalar/r2cb.h
 */
...
static void r2cb_14(R *R0, R *R1, R *Cr, R *Ci, stride rs, stride csr,
  ...
  DK(KP554958132, +0.554958132087371191422194871006410481067288862);
  {
    ...
    for (i = v; i > 0; i = i - 1, R0 = R0 + ovs, R1 = R1 + ovs, Cr = Cr + ivs,
        Ci = Ci + ivs, MAKE_VOLATILE_STRIDE(56, rs),
  ...
  DK(KP2_000000000, +2.000000000000000000000000000000000000000000000);
  {
    ...
    for (i = v; i > 0; i = i - 1, R0 = R0 + ovs, R1 = R1 + ovs, Cr = Cr + ivs,
      ...
      R0[0] = FMA(KP2_000000000, Te + Tf + Tg, Td);
      {
        ...
        Th = FMA(KP1_246979603, Tf, Td) +
             FNMA(KP445041867, Tg, KP1_801937735 * Te);
        R0[WS(rs, 2)] = Th - Tr;
        R0[WS(rs, 5)] = Th + Tr;
        TE = FMA(KP867767478, Tx, KP1_563662964 * Ty) - (KP1_949855824 * Tz);
        TD = FMA(KP1_246979603, Tc, T3) +
             FNMA(KP1_801937735, T9, KP445041867 * T6);
        R1[WS(rs, 2)] = TD - TE;
        R1[WS(rs, 4)] = TD + TE;
      }
      {
        ...
        Ts = FMA(KP1_246979603, Tg, Td) +
             FNMA(KP1_801937735, Tf, KP445041867 * Te);
        R0[WS(rs, 6)] = Ts - Tt;
        R0[WS(rs, 1)] = Ts + Tt;
        TA = FNMS(KP1_949855824, Ty, KP1_563662964 * Tx) - (KP867767478 * Tz);
        Tw = FMA(KP1_246979603, T9, T3) +
             FNMA(KP445041867, Tc, KP1_801937735 * T6);
        R1[WS(rs, 5)] = Tw - TA;
        R1[WS(rs, 1)] = Tw + TA;
      }
      {
        ...
        TB = FMA(KP1_246979603, T6, T3) +
             FNMA(KP1_801937735, Tc, KP445041867 * T9);
        R1[0] = TB - TC;
        R1[WS(rs, 6)] = TB + TC;
        Tv = FMA(KP1_563662964, Tq, KP1_949855824 * Tk) + (KP867767478 * Tn);
        Tu = FMA(KP1_246979603, Te, Td) +
             FNMA(KP1_801937735, Tg, KP445041867 * Tf);
        R0[WS(rs, 4)] = Tu - Tv;
        R0[WS(rs, 3)] = Tu + Tv;
      }
    }
