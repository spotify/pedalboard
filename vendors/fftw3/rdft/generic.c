...
static void print(const plan *ego_, printer *p) {
  ...
  p->print(p, "(rdft-generic-%s-%D)", ego->kind == R2HC ? "r2hc" : "hc2r",
           ego->n);
}
...
static int applicable(const S *ego, const problem *p_, const planner *plnr) {
  ...
  return (
      ...
      X(is_prime)(p->sz->dims[0].n) && p->kind[0] == ego->kind);
}
...
static plan *mkplan(const solver *ego_, const problem *p_, planner *plnr) {
  ...
  static const plan_adt padt = {X(rdft_solve), awake, print,
                                X(plan_null_destroy)};
  ...
  pln = MKPLAN_RDFT(P, &padt, R2HC_KINDP(p->kind[0]) ? apply_r2hc : apply_hc2r);
