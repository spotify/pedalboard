...
void X(rdft_zerotens)(tensor *sz, R *I) { recur(sz->dims, sz->rnk, I); }

#define KSTR_LEN 8

const char *X(rdft_kind_str)(rdft_kind kind) {
  static const char kstr[][KSTR_LEN] = {
      "r2hc",    "r2hc01",  "r2hc10",  "r2hc11",  "hc2r",    "hc2r01",
      "hc2r10",  "hc2r11",  "dht",     "redft00", "redft01", "redft10",
      "redft11", "rodft00", "rodft01", "rodft10", "rodft11"};
...
static void print(const problem *ego_, printer *p) {
  ...
  int i;
  p->print(p, "(rdft %d %D %T %T", X(ialignment_of)(ego->I),
           (INT)(ego->O - ego->I), ego->sz, ego->vecsz);
...
static const problem_adt padt = {PROBLEM_RDFT, hash, zero, print, destroy};
...
problem *X(mkproblem_rdft)(const tensor *sz, const tensor *vecsz, R *I, R *O,
  ...
  ego = (problem_rdft *)X(mkproblem)(
      sizeof(problem_rdft) + sizeof(rdft_kind) * (rnk > 0 ? rnk - 1u : 0u),
      &padt);
#elif defined(STRUCT_HACK_C99)
  ego = (problem_rdft *)X(mkproblem)(
      sizeof(problem_rdft) + sizeof(rdft_kind) * (unsigned)rnk, &padt);
#else
  ...
  for (i = 0; i < rnk; ++i)
    if (ego->sz->dims[i].n == 2 &&
        (ego->kind[i] == REDFT00 || ego->kind[i] == DHT ||
         ego->kind[i] == HC2R))
