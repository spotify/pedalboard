...
static void print(const plan *ego_, printer *p) {
  ...
  p->print(p, "(rdft2-rdft-%s-%D%v/%D-%D%(%p%)%(%p%))",
           ego->super.apply == apply_r2hc ? "r2hc" : "hc2r", ego->n, ego->nbuf,
           ego->vl, ego->bufdist % ego->n, ego->cld, ego->cldrest);
}
...
static INT min_nbuf(const problem_rdft2 *p, INT n, INT vl) {
  ...
  if (n * X(iabs)(is) <= X(iabs)(ivs) &&
      ...
      (((p->cr - p->ci) <= X(iabs)(os)) || ((p->ci - p->cr) <= X(iabs)(os))) &&
      ivs > 0 && ovs > 0) {
...
static plan *mkplan(const solver *ego_, const problem *p_, planner *plnr) {
  ...
  if (p->kind == R2HC) {
    cld = X(mkplan_f_d)(
        plnr,
        X(mkproblem_rdft_d)(X(mktensor_1d)(n, p->sz->dims[0].is / 2, 1),
    ...
    cld = X(mkplan_f_d)(
        plnr,
        X(mkproblem_rdft_d)(X(mktensor_1d)(n, 1, p->sz->dims[0].os / 2),
                            X(mktensor_1d)(nbuf, bufdist, ovs), bufs,
                            TAINT(p->r0, ovs * nbuf), &p->kind),
  ...
  X(ops_madd)(vl / nbuf, &cld->ops, &cldrest->ops, &pln->super.super.ops);
...
void X(rdft2_rdft_register)(planner *p) { REGISTER_SOLVER(p, mksolver()); }
