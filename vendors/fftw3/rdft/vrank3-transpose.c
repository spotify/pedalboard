...
typedef struct {
  rdftapply apply;
  int (*applicable)(const problem_rdft *p, planner *plnr, int dim0, int dim1,
                    int dim2, INT *nbuf);
...
static int Ntuple_transposable(const iodim *a, const iodim *b, INT vl, INT vs) {
  return (
      vs == 1 && b->is == vl && a->os == vl &&
      ((a->n == b->n && a->is == b->os && a->is >= b->n && a->is % vl == 0) ||
...
static int applicable_gcd(const problem_rdft *p, planner *plnr, int dim0,
                          int dim1, int dim2, INT *nbuf) {
  ...
  *nbuf = n * (m / d) * vl;
  return (!NO_SLOWP(plnr) /* FIXME: not really SLOW for large 1d ffts */
          && n != m && d > 1 &&
          Ntuple_transposable(p->vecsz->dims + dim0, p->vecsz->dims + dim1, vl,
                              vs));
}
...
static int mkcldrn_gcd(const problem_rdft *p, planner *plnr, P *ego) {
  ...
  if (n > 1) {
    ego->cld1 = X(mkplan_d)(
        plnr,
        X(mkproblem_rdft_0_d)(X(mktensor_3d)(n, d * m * vl, m * vl, d, m * vl,
                                             n * m * vl, m * vl, 1, 1),
                              TAINT(p->I, num_el), buf));
    if (!ego->cld1)
      goto nada;
    X(ops_madd)
    (d, &ego->cld1->ops, &ego->super.super.ops, &ego->super.super.ops);
    ego->super.super.ops.other += num_el * d * 2;
  }

  ego->cld2 = X(mkplan_d)(
      plnr, X(mkproblem_rdft_0_d)(X(mktensor_3d)(d, d * n * m * vl, n * m * vl,
                                                 d, n * m * vl, d * n * m * vl,
                                                 n * m * vl, 1, 1),
                                  p->I, p->I));
  if (!ego->cld2)
  ...
  if (m > 1) {
    ego->cld3 = X(mkplan_d)(
        plnr, X(mkproblem_rdft_0_d)(X(mktensor_3d)(d * n, m * vl, vl, m, vl,
                                                   d * n * vl, vl, 1, 1),
                                    TAINT(p->I, num_el), buf));
    if (!ego->cld3)
      goto nada;
...
static const transpose_adt adt_gcd = {apply_gcd, applicable_gcd, mkcldrn_gcd,
                                      "rdft-transpose-gcd"};
...
static int applicable_cut(const problem_rdft *p, planner *plnr, int dim0,
                          int dim1, int dim2, INT *nbuf) {
  INT n = p->vecsz->dims[dim0].n;
  INT m = p->vecsz->dims[dim1].n;
  INT vl, vs;
  ...
  *nbuf = 0;                 /* always small enough to be non-UGLY (?) */
  ...
  return (!NO_SLOWP(plnr)    /* FIXME: not really SLOW for large 1d ffts? */
          ...
          && (cut1(n, m, vl) || gcd(n, m) < X(imin)(MINBUFDIV, X(imin)(n, m)))

          && Ntuple_transposable(p->vecsz->dims + dim0, p->vecsz->dims + dim1,
                                 vl, vs));
}
...
static int mkcldrn_cut(const problem_rdft *p, planner *plnr, P *ego) {
  ...
  if (m > mc) {
    ego->cld1 = X(mkplan_d)(
        plnr, X(mkproblem_rdft_0_d)(
                  X(mktensor_3d)(nc, m * vl, vl, m - mc, vl, nc * vl, vl, 1, 1),
                  p->I + mc * vl, buf));
    if (!ego->cld1)
      goto nada;
  ...
  ego->cld2 = X(mkplan_d)(
      plnr, X(mkproblem_rdft_0_d)(
                X(mktensor_3d)(nc, mc * vl, vl, mc, vl, nc * vl, vl, 1, 1),
                p->I, p->I));
  if (!ego->cld2)
    goto nada;
  X(ops_add2)(&ego->cld2->ops, &ego->super.super.ops);
  ...
  if (n > nc) {
    ego->cld3 = X(mkplan_d)(
        plnr, X(mkproblem_rdft_0_d)(
                  X(mktensor_3d)(n - nc, m * vl, vl, m, vl, n * vl, vl, 1, 1),
  ...
  ego->super.super.ops.other +=
      2 * vl * (nc * mc * ((m > mc) + (n > nc)) + (n - nc) * m + (m - mc) * nc);
...
static const transpose_adt adt_cut = {apply_cut, applicable_cut, mkcldrn_cut,
                                      "rdft-transpose-cut"};
...
static void transpose_toms513(R *a, INT nx, INT ny, INT N, char *move,
                              INT move_size, R *buf) {
  INT i, im, mn;
  R *b, *c, *d;
  INT ncount;
  ...
  while (1) {
    ...
    while (1) {
      ...
      case 2:
        ...
        break;
      default:
        memcpy(&a[N * i1], &a[N * i2], N * sizeof(R));
        memcpy(&a[N * i1c], &a[N * i2c], N * sizeof(R));
      }
      i1 = i2;
      i1c = i2c;
    }
...
static int applicable_toms513(const problem_rdft *p, planner *plnr, int dim0,
                              int dim1, int dim2, INT *nbuf) {
  INT n = p->vecsz->dims[dim0].n;
  INT m = p->vecsz->dims[dim1].n;
  INT vl, vs;
  get_transpose_vec(p, dim2, &vl, &vs);
  *nbuf = 2 * vl + ((n + m) / 2 * sizeof(char) + sizeof(R) - 1) / sizeof(R);
  return (!NO_SLOWP(plnr) && (vl > 8 || !NO_UGLYP(plnr)) /* UGLY for small vl */
          && n != m &&
          Ntuple_transposable(p->vecsz->dims + dim0, p->vecsz->dims + dim1, vl,
                              vs));
}
...
static const transpose_adt adt_toms513 = {apply_toms513, applicable_toms513,
                                          mkcldrn_toms513,
...
static void print(const plan *ego_, printer *p) {
  const P *ego = (const P *)ego_;
  p->print(p, "(%s-%Dx%D%v", ego->slv->adt->nam, ego->n, ego->m, ego->vl);
  if (ego->cld1)
    p->print(p, "%(%p%)", ego->cld1);
  if (ego->cld2)
    p->print(p, "%(%p%)", ego->cld2);
  if (ego->cld3)
    p->print(p, "%(%p%)", ego->cld3);
  p->print(p, ")");
}
...
static plan *mkplan(const solver *ego_, const problem *p_, planner *plnr) {
  ...
  static const plan_adt padt = {X(rdft_solve), awake, print, destroy};
...
void X(rdft_vrank3_transpose_register)(planner *p) {
  unsigned i;
  static const transpose_adt *const adts[] = {&adt_gcd, &adt_cut, &adt_toms513};
