...
static void os_create_thread(FFTW_WORKER (*worker)(void *arg), void *arg) {
  pthread_attr_t attr;
  pthread_t tid;
...
static void os_destroy_thread(void) { pthread_exit((void *)0); }
...
static void os_mutex_destroy(os_mutex_t *s) { CloseHandle(*s); }

static void os_mutex_lock(os_mutex_t *s) { WaitForSingleObject(*s, INFINITE); }

static void os_mutex_unlock(os_mutex_t *s) { ReleaseMutex(*s); }
...
static void os_sem_destroy(os_sem_t *s) { CloseHandle(*s); }

static void os_sem_down(os_sem_t *s) { WaitForSingleObject(*s, INFINITE); }

static void os_sem_up(os_sem_t *s) { ReleaseSemaphore(*s, 1, NULL); }

#define FFTW_WORKER unsigned __stdcall
typedef unsigned(__stdcall *winthread_start)(void *);

static void os_create_thread(winthread_start worker, void *arg) {
...
static void os_destroy_thread(void) { _endthreadex(0); }
...
int X(ithreads_init)(void) {
  os_static_mutex_lock(&initialization_mutex);
  {
    ...
    WITH_QUEUE_LOCK({ worker_queue = 0; });
  }
...
void X(spawn_loop)(int loopmax, int nthr, spawn_function proc, void *data) {
  ...
  if (X(spawnloop_callback)) { /* user-defined spawnloop backend */
    ...
    X(spawnloop_callback)
    (proc, sdata, sizeof(spawn_data), nthr, X(spawnloop_callback_data));
    STACK_FREE(sdata);
  } else {
    struct work *r;
...
static os_static_mutex_t install_planner_hooks_mutex =
    OS_STATIC_MUTEX_INITIALIZER;
static os_mutex_t planner_mutex;
static int planner_hooks_installed = 0;

static void lock_planner_mutex(void) { os_mutex_lock(&planner_mutex); }

static void unlock_planner_mutex(void) { os_mutex_unlock(&planner_mutex); }
