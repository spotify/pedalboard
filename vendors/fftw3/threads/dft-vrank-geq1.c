...
static void *spawn_apply(spawn_data *d) {
  ...
  cld->apply((plan *)cld, ego->ri + thr_num * its, ego->ii + thr_num * its,
...
static int applicable0(const solver *ego_, const problem *p_,
  ...
  return (1 && plnr->nthr > 1 && FINITE_RNK(p->vecsz->rnk) &&
          p->vecsz->rnk > 0 && pickdim(ego, p->vecsz, p->ri != p->ro, dp));
}
...
static plan *mkplan(const solver *ego_, const problem *p_, planner *plnr) {
  ...
  for (i = 0; i < nthr; ++i) {
    ...
    cldp = X(mkproblem_dft)(p->sz, vecsz, p->ri + i * its, p->ii + i * its,
