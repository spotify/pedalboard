...
static void *spawn_apply(spawn_data *d) {
  ...
  cld->apply((plan *)cld, ego->I + thr_num * ego->its,
             ego->O + thr_num * ego->ots);
  return 0;
}
...
static int applicable0(const solver *ego_, const problem *p_,
  ...
  return (1 && plnr->nthr > 1 && FINITE_RNK(p->vecsz->rnk) &&
          p->vecsz->rnk > 0 && pickdim(ego, p->vecsz, p->I != p->O, dp));
}
...
static int applicable(const solver *ego_, const problem *p_,
  ...
  if (!applicable0(ego_, p_, plnr, dp))
    return 0;
...
static plan *mkplan(const solver *ego_, const problem *p_, planner *plnr) {
  ...
  for (i = 0; i < nthr; ++i) {
    ...
    cldp = X(mkproblem_rdft)(p->sz, vecsz, p->I + i * its, p->O + i * ots,
                             p->kind);
