...
static void *spawn_apply(spawn_data *d) {
  ...
  cld->apply((plan *)cld, ego->r0 + thr_num * its, ego->r1 + thr_num * its,
...
static int applicable0(const solver *ego_, const problem *p_,
  ...
  if (FINITE_RNK(p->vecsz->rnk) && p->vecsz->rnk > 0 && plnr->nthr > 1 &&
...
static plan *mkplan(const solver *ego_, const problem *p_, planner *plnr) {
  ...
  for (i = 0; i < nthr; ++i) {
    ...
    cldp = X(mkproblem_rdft2)(p->sz, vecsz, p->r0 + i * its, p->r1 + i * its,
                              p->cr + i * ots, p->ci + i * ots, p->kind);
