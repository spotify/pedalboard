...
#define VLIT(x0, x1)                                                           \
  DS(_mm256_set_pd(x0, x1, x0, x1),                                            \
     _mm256_set_ps(x0, x1, x0, x1, x0, x1, x0, x1))
...
static inline V VZMULI(V tx, V sr) {
   ...
   * (2 permute, 1 shuffle, 1 constant load (L1), 1 xor, 2 fp), since the below
   * FMA one would be 2 permute, 1 shuffle, 1 xor (setzero), 3 fp), but with a
   * longer pipeline.
   *
   * Alternative new fma version:
   * return SUFF(_mm256_addsub_p)(SUFF(_mm256_fnmadd_p)(sr, VDUPH(tx),
   * SUFF(_mm256_setzero_p)()), VMUL(FLIP_RI(sr), VDUPL(tx)));
   */
}
