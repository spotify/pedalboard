...
static void awake(plan *ego_, enum wakefulness wakefulness) {
  ...
  static const tw_instr reodft11e_tw[] = {
      {TW_COS, 1, 1}, {TW_SIN, 1, 1}, {TW_NEXT, 2, 0}};

  X(plan_awake)(ego->cld, wakefulness);

  X(twiddle_awake)
  (wakefulness, &ego->td, reodft010e_tw, 2 * ego->n, 1, ego->n / 4 + 1);
  X(twiddle_awake)(wakefulness, &ego->td2, reodft11e_tw, 8 * ego->n, 1, ego->n);
}
...
static void print(const plan *ego_, printer *p) {
  const P *ego = (const P *)ego_;
  p->print(p, "(%se-radix2-r2hc-%D%v%(%p%))", X(rdft_kind_str)(ego->kind),
