...
static void print(const plan *ego_, printer *p) {
  const P *ego = (const P *)ego_;
  p->print(p, "(redft00e-r2hc-pad-%D%v%(%p%)%(%p%))", ego->n + 1, ego->vl,
           ego->cld, ego->cldcpy);
}
...
static int applicable0(const solver *ego_, const problem *p_) {
  ...
  return (1 && p->sz->rnk == 1 && p->vecsz->rnk <= 1 && p->kind[0] == REDFT00 &&
...
static plan *mkplan(const solver *ego_, const problem *p_, planner *plnr) {
  ...
  cld = X(mkplan_d)(plnr,
                    X(mkproblem_rdft_1_d)(X(mktensor_1d)(2 * n, 1, 1),
  ...
  cldcpy = X(mkplan_d)(
      plnr, X(mkproblem_rdft_1_d)(X(mktensor_0d)(),
                                  X(mktensor_1d)(n + 1, 1, p->sz->dims[0].os),
