...
static void awake(plan *ego_, enum wakefulness wakefulness) {
  P *ego = (P *)ego_;
  static const tw_instr rodft00e_tw[] = {{TW_SIN, 0, 1}, {TW_NEXT, 1, 0}};

  X(plan_awake)(ego->cld, wakefulness);

  X(twiddle_awake)
  (wakefulness, &ego->td, rodft00e_tw, 2 * ego->n, 1, (ego->n + 1) / 2);
}
...
static int applicable0(const solver *ego_, const problem *p_) {
  ...
  return (1 && p->sz->rnk == 1 && p->vecsz->rnk <= 1 && p->kind[0] == RODFT00);
}
...
void X(rodft00e_r2hc_register)(planner *p) { REGISTER_SOLVER(p, mksolver()); }
