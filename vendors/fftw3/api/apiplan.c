...
static plan *mkplan0(planner *plnr, unsigned flags, const problem *prb,
                     unsigned hash_info, wisdom_state_t wisdom_state) {
...
static plan *mkplan(planner *plnr, unsigned flags, const problem *prb,
                    unsigned hash_info) {
  plan *pln;
  ...
  if (plnr->wisdom_state == WISDOM_NORMAL && !pln) {
    ...
    pln = mkplan0(plnr, force_estimator(flags), prb, hash_info,
...
apiplan *X(mkapiplan)(int sign, unsigned flags, problem *prb) {
  ...
  planner *plnr;
  static const unsigned int pats[] = {FFTW_ESTIMATE, FFTW_MEASURE, FFTW_PATIENT,
                                      FFTW_EXHAUSTIVE};
  int pat, pat_max;
  double pcost = 0;
  ...
  if (flags & FFTW_WISDOM_ONLY) {
    ...
    pat_max =
        flags & FFTW_ESTIMATE
            ? 0
            : (flags & FFTW_EXHAUSTIVE ? 3 : (flags & FFTW_PATIENT ? 2 : 1));
...
int X(alignment_of)(R *p) { return X(ialignment_of(p)); }
