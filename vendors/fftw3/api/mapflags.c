...
#define NO(x)                                                                  \
  { x, x }
#define IMPLIES(predicate, consequence)                                        \
...
void X(mapflags)(planner *plnr, unsigned flags) {
  ...
  const flagop self_flagmap[] = {
      ...
      */
      ...
      IMPLIES(NO(FFTW_EXHAUSTIVE), YES(FFTW_NO_SLOW)),

      /* a canonical set of fftw2-like impatience flags */
      IMPLIES(NO(FFTW_PATIENT),
              YES(FFTW_NO_VRECURSE | FFTW_NO_RANK_SPLITS |
                  FFTW_NO_VRANK_SPLITS | FFTW_NO_NONTHREADED |
                  FFTW_NO_DFT_R2HC | FFTW_NO_FIXED_RADIX_LARGE_N |
                  FFTW_BELIEVE_PCOST))};

  /* map of (processed) api flags to internal problem/planner flags */
  const flagop l_flagmap[] = {EQV(FFTW_PRESERVE_INPUT, NO_DESTROY_INPUT),
  ...
  const flagop u_flagmap[] = {
      ...
      EQV(FFTW_NO_VRECURSE, NO_VRECURSE), EQV(FFTW_NO_SLOW, NO_SLOW),
